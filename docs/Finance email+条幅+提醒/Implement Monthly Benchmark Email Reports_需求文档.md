...
**含义**：
当公司首次产生新的closed month财务数据时触发邮件生成，始终与上封邮件的百分位比较
- 背景补充,closed month定义： 
...
**触发时机**：
- 每日定时检测，有新closed month时发送，指标始终与上封邮件的百分位比较
  
**发送对象**：
- Company Admin（公司管理员）：仅包含本公司的信息，每次触发都收邮件，始终与上封邮件的百分位比较，无静默更新场景
- 该公司所在的Portfolio的Portfolio manager：接收投资组合内有重要变化的邮件汇总，始终与上封邮件的百分位比较，未达重要变化阈值不收邮件时触发静默更新
...
**发送对象**：
- Company Admin：仅包含本公司的信息，每次触发都收邮件，始终与上封邮件的百分位比较，无静默更新场景
- Portfolio Manager：接收投资组合内有重要变化的邮件汇总，始终与上封邮件的百分位比较，未达重要变化阈值不收邮件时触发静默更新

### 2.邮件内容范围

#### 2.1 Company Admin邮件内容范围：

- 仅包含该公司的信息，指标百分位变化信息用触发时的当前closed month实时百分位，减去系统预先存储的上一个[closed month的）基准百分位]。
- 包含所有 6 个基准指标的详细数据
- 邮件发出时 → 记录当前百分位为该收件人新基准记录

**特殊处理**
- 首封邮件，无法计算变化幅度，25号或新的closed month产生，直接发送并记录当前百分位为基准。
- 下次计算变化幅度时，与"最近一次存储的基准"对比，无论基准是邮件触发还是静默更新
- 邮件发出时 → 记录当前百分位为该收件人新基准记录
  
#### 2.2 Portfolio manager邮件内容范围：

- 包括该Potfolio下所有有重大变化的公司，指标百分位变化信息用触发时的当前closed month实时百分位，减去系统预先存储的上一个[closed month的）基准百分位]。
- 仅展示有重大变化的指标
- 邮件发出时 → 记录当前百分位为该收件人新基准记录

**特殊处理**
- 首封邮件，无法计算变化幅度，25号或新的closed month产生，直接发送并记录当前百分位为基准。PM 首封邮件展示所有公司快照，不做重要变化阈值过滤。
- 新 closed month 入库但未达重要变化阈值PM 未收到该公司的邮件 → 静默更新
- Closed month 数据被修订但未达重要变化阈值PM 未收到该公司的邮件  → 静默更新基准
- 下次计算变化幅度时，与"最近一次存储的基准"对比，无论基准是邮件触发还是静默更新
- 邮件发出时 → 记录当前百分位为该收件人新基准记录
