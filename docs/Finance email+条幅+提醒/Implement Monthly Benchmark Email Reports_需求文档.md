...
- 仅包含该公司的信息，指标百分位变化信息用触发时的当前closed month实时百分位，减去系统预先存储的上一个基准百分位（closed month的）。
- 包含所有 6 个基准指标的详细数据
...
- 包括该Potfolio下所有有重大变化的公司，指标百分位变化信息用触发时的当前closed month实时百分位，减去系统预先存储的上一个基准百分位（closed month的）。
- 仅展示有重大变化的指标
- 邮件发出时 → 记录当前百分位为该收件人新基准记录
