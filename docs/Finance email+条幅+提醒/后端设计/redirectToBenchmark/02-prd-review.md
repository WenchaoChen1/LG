...
## 七、已自动修订项（2026-04-22 更新）

**状态**：用户已逐条回复 Q-A ~ Q-F，原评审的 11 条 P0 全部闭环；`01-prd.md` 已重写为 v2.0，补齐 SC/BR/AC 编号、权限矩阵、错误处理与验收标准。

**本轮不再叫"自动修订"——是在用户澄清基础上的结构化重写**。非决策类精简（删套话、统一术语、格式修正）已随重写一并完成。

| # | 位置 | 修订内容 | 类型 | 解决哪条原 P0 |
|---|------|----------|------|----------------|
| 1 | 全文 | 新增 SC-01/02/03/04、BR-01~BR-10、AC-01~AC-09 完整编号 | 编号补全 | #1 |
| 2 | 全文 | "Company Benchmarking页面" → 不再出现（用户明确本次不含前端，后端不关心页面名） | 术语统一 | #2 |
| 3 | §二 SC-01 / §十 | 触发点明确为 `FinanceManualDataService.save()` / `FinancialSummaryServiceImpl.acceptChanges()`；close_month 复用 `resolveCloseMonthDate` | 术语/归口 | #3、#4 |
| 4 | §八 | 新增"错误处理与边界情况"表（含并发、null、回退、重复等 7 个场景） | 状态补充 | #5 |
| 5 | §六 6.1 | 移除"Actuals – Internal Peers 视图"这类前端过滤器描述；后端不关心 | 范围精简 | #6 |
| 6 | §七 BR-03 | 条幅文案由用户定版（保留原文） | 文案定版 | #7 |
| 7 | §一 / §二 | 明确"条幅 = Toast = 一条通知"、本次只记录后端事件，前端如何呈现不由 PRD 约束 | 机制澄清 | #8 |
| 8 | §六 / §四 | 通知持久化方案敲定：`benchmark_notification_record` UNIQUE(company_id) + `benchmark_notification_dismiss` FK CASCADE | 机制澄清 | #9 |
| 9 | §七 BR-03 | 条幅文案作为常量返回，未引入变量占位符（用户定版文案即如此） | 文案定版 | #10 |
| 10 | §九 | 权限矩阵明确：不分 roleType，复用公司访问权限 | 权限归口 | #11 |
| 11 | §七 BR-10 | 触发连接器白名单：仅 MANUAL / QBO | 范围收敛 | #14 |
| 12 | §一 / §十三 | 明确"本期不含前端代码" | 范围收敛 | — |

**非阻塞类（仍保留的改进空间，留给下一轮**：
- BR-03 文案"for have been"语法空档，用户确认本期保留原文（见 TDD R5）
- 性能 SLO 仅 API-01 有 ≤100ms，API-02 未显式给；实现后补
- 审计与监控指标未定（是否上报 `benchmark_notification_replaced` 事件到 Sentry）
