...
- **⑦ SpreadJS** — ✅ "花钱买零技术风险"，四件套（切 sheet / 选区 / 滚动 / 高亮）官方齐备且纯前端导入。❌ $12,499/年 部署授权是报价单上不显眼的一项，每个 hostname（dev/test/uat/prod）都要 key，无 key 只能 localhost（且**即使在 localhost 也会插入一个 `Evaluation Version` sheet 页 + 画布水印**，2026-09-04 实测，比官方文档描述的"右下角水印"显眼得多）；`.xls` 需服务端先转。
  - **两条 xlsx 导入路线的图片差异（2026-09-04 实测，POC 页 `/devSupport/excelPreviewDemo` 双路线对比）**：官方现推荐的 `spread.import()`（`@mescius/spread-sheets-io`）**丢图片**——含 1 个 `<xdr:pic>` 浮动图片的 xlsx 导入后 `sheet.pictures` 计数为 0、页面上也看不到；改走旧路线 `ExcelIO.open()`（`@mescius/spread-excelio`）+ `importPictureAsFloatingObject: true` 后**图片正常显示**。该选项官方注释原文是 "import picture as floatingObject **instead of shape**"，即这条链默认把图片当 shape 导（shape 渲染另需 `@mescius/spread-sheets-shapes`）。
  - **但换路线有代价**：`ImportXlsxOptions` 有 14 个字段（含 `openMode` 懒加载/增量、`progress` 进度回调、`calcOnDemand`、`incrementalCalculation`），而 `ExcelIO.OpenOptions` **只有 2 个**（`password` + `importPictureAsFloatingObject`），且 excelio 已不在官方 Getting Started 的包清单里、额外 +305 KB gzip。大文件（6 MB / ~15 万单元格量级）正是靠 `openMode` + `progress` 缓解阻塞的，走 excelio 就没有这两个工具。**取舍口径：财报里的图片多为 logo / 导出说明截图，与数据核对无关；大文件卡顿是可感知的体验问题。故主链路建议保 `-io`，图片按需再议**——除非产品明确要求预览与原文件一致。
