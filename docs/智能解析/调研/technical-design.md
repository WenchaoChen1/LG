...
| `ai_ocr_conflict_record` | 冲突检测结果 | task_id, company_id, document_type, resolution, user_note |
| `mapping_memory` | 两层映射记忆（通用+公司） | company_id, source_term, normalized_category, confidence, is_trusted |
| `mapping_memory_audit` | 记忆变更审计日志 | mapping_id, event_type, old_category, new_category, actor |
