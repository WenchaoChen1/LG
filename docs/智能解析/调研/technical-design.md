...
│                                                │
│  ┌────────────────────────────────────────┐    │
│  │ PostgreSQL (ai_ocr_company_mapping_memory) │    │
...
第 1 次上传（某 SaaS 公司）
  ...
  用户确认 ✓
  → 保存到公司记忆 (ai_ocr_company_mapping_memory)
...
这种方式零额外依赖，利用已有的 `ai_ocr_company_mapping_memory` 表和 `company.industry` 字段即可实现跨公司经验共享。
