...
│       │   └── DocParseConfirmReqVo.java  # 确认提交请求 VO
│       └── response/
...
│   │   └── DocParseCommitService.java      # 写入 fi_* + 触发记忆学习 SQS
│   └── dto/
...
│   │   └── DocParseFileRepository.java     # JpaRepository（含 findByCompanyIdAndFileHash 用于重名检测）
│   └── enums/
