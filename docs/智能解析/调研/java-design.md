...
CREATE TABLE doc_parse_task (
    ...
    status          VARCHAR(30) NOT NULL DEFAULT 'UPLOADING',
        -- UPLOADING / UPLOAD_COMPLETE / PROCESSING / REVIEWING / 
        -- VERIFYING / CONFLICT_RESOLUTION / COMMITTING / COMPLETED / FAILED
    total_files     INT NOT NULL DEFAULT 0,
    completed_files INT NOT NULL DEFAULT 0,
    failed_files    INT NOT NULL DEFAULT 0,
    completed_at    TIMESTAMPTZ,                                      -- 全部完成时间（批次完成）
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX idx_doc_parse_task_status
    ON doc_parse_task (company_id, status, created_at DESC);
```

**状态机（Task 级）**:

```
UPLOADING → UPLOAD_COMPLETE → PROCESSING → REVIEWING → VERIFYING → CONFLICT_RESOLUTION → COMMITTING → COMPLETED
                                                                                                          ↓
任何环节异常 → FAILED
```

**状态推进规则**（在 DocParseStatusService 中）:
- UPLOADING → UPLOAD_COMPLETE: 所有文件 ≥ UPLOADED（实际跳过，因为发 SQS 后直接进 PROCESSING）
- → PROCESSING: 至少一个文件 status=PROCESSING
- → REVIEWING: **所有** files.status = REVIEW_READY（批次完成标志）
- → COMMITTING: 用户点 Commit
- → COMPLETED: **所有** files.status = FILE_COMMITTED（批次完成）
- → FAILED: 任何 commit 步骤失败（事务回滚）
...
CREATE TABLE doc_parse_file (
    ...
    company_id      BIGINT NOT NULL,                             -- 冗余存储，支持按公司唯一约束
    filename        VARCHAR(500) NOT NULL,
    file_type       VARCHAR(10) NOT NULL,
    file_size       BIGINT NOT NULL,
    file_hash       VARCHAR(64) NOT NULL,                        -- SHA-256，用于重名校验
    ...
    processing_stage VARCHAR(20),                                 -- 仅 status=PROCESSING 有效
        -- EXTRACTING / MAPPING_RULE / MAPPING_MEMORY / 
        -- MAPPING_LLM / VALIDATING / PERSISTING
    progress_pct    SMALLINT DEFAULT 0,                          -- 0-100，Python 进度上报
    upload_error    VARCHAR(20),                                  -- 5 种上传错误枚举
        -- FILE_TOO_LARGE / TYPE_NOT_SUPPORTED / CORRUPTED / 
        -- BATCH_TOO_LARGE / DUPLICATE_NAME
    error_message   TEXT,
...
|-------|------------|---------------|--------------|---------------|--------------|---------------|
| t0 用户选择 3 个文件 | — | PENDING | — | PENDING | — | PENDING |
| t1 Java 校验+S3 | UPLOADING | UPLOADING→UPLOADED | — | UPLOADING→UPLOADED | — | FILE_FAILED（重名） |
| t2 发 SQS | PROCESSING | QUEUED | — | QUEUED | — | FILE_FAILED |
...
| t6 file_2 完成 | **REVIEWING** | REVIEW_READY | — | REVIEW_READY | — | FILE_FAILED |
| t7 用户 verify | VERIFYING | REVIEW_READY | — | REVIEW_READY | — | FILE_FAILED |
| t8 用户 commit | COMMITTING | FILE_COMMITTED | — | FILE_COMMITTED | — | FILE_FAILED |
| t9 全部完成 | **COMPLETED** | FILE_COMMITTED | — | FILE_COMMITTED | — | FILE_FAILED |

**关键点**:
- **t6**: 尽管 file_3 失败了，只要**非失败文件都 REVIEW_READY**，task 进入 REVIEWING
- **t9**: COMPLETED 不要求所有 file 成功，只要求所有**非失败文件**都 FILE_COMMITTED
- **后置动作**（邮件、记忆学习）在 t9 COMPLETED 触发**一次**，不会因 file_3 FILE_FAILED 而跳过
...
Python 端在处理过程中向 `ocr-result-queue` 发送**两类**消息。Java 端 `OcrResultSqsProcessor`（implements `MessageProcessor`，由 `MessageProcessorManager` 在 `@PostConstruct` 阶段自动扫描注册）消费此消息，更新 `doc_parse_file` 和 `doc_parse_task` 表状态。

#### 4.2.1 OcrProgress 消息（进度上报，轻量，频繁）
...
**Java 处理**:
- 更新 `doc_parse_file.processing_stage` 和 `progress_pct`
- 首次收到（file.status=QUEUED）时 → file.status=PROCESSING，task.status=PROCESSING
- 不触发 task 状态转换（仅 OcrResult 才触发）

#### 4.2.2 OcrResult 消息（最终结果，每个文件一次）
...
if (msg.status == "completed") {
    ...
    file.processing_stage = null;
    file.progress_pct = 100;
} else {
    file.status = FILE_FAILED;
    file.error_message = msg.error;
    task.failed_files++;
}

// 检查 batch 是否完成
if (所有非 FAILED 文件.status = REVIEW_READY) {
    task.status = REVIEWING;  // ← 批次完成进入审核
}
```
