...
> **关联文档**: [数据库 Schema](./database-schema.md) · [系统架构](./system-architecture.md) · [Java 端设计](./java-design.md) · [Python 端设计](./python-design.md) · [前端设计](./frontend-design.md) · [需求分析](./requirement-analysis.md) · [代码示例](./code-examples.md)
...
- [一、核心论点](#一核心论点)
- [二、六项设计原则](#二六项设计原则)
  - [原则 1：确定性优先于智能](#原则-1确定性优先于智能)
  - [原则 2：Agent 拥有自己的边界](#原则-2agent-拥有自己的边界)
  - [原则 3：AI 成本是产品决策，不是事后优化](#原则-3ai-成本是产品决策不是事后优化)
  - [原则 4：人工审核不可协商](#原则-4人工审核不可协商)
  - [原则 5：记忆优于训练](#原则-5记忆优于训练)
  - [原则 6：为今天的 Agent 构建，为明天的系统留接口](#原则-6为今天的-agent-构建为明天的系统留接口)
- [三、原始需求问题与对应解决策略](#三原始需求问题与对应解决策略)
- [四、关于关键技术选择的立场](#四关于关键技术选择的立场)
  - [AI Vision 替代传统 OCR（eSapiens）](#ai-vision-替代传统-ocresapiens)
  - [Instructor + Pydantic 替代 LangChain Parser](#instructor--pydantic-替代-langchain-parser)
  - [pgvector 替代专用向量数据库（含 RAG 阶段立场）](#pgvector-替代专用向量数据库含-rag-阶段立场)
  - [LangGraph 替代"纯 FastAPI 端点"](#langgraph-替代纯-fastapi-端点)
- [五、我们明确不做的事](#五我们明确不做的事)
- [六、衡量成功的指标](#六衡量成功的指标)
- [七、2026-04-20 新增原则（项目内部补丁）](#七2026-04-20-新增原则项目内部补丁)
  - [原则 7：所有长耗时异步状态必须落库](#原则-7所有长耗时异步状态必须落库)
  - [原则 8：任何业务对象都必须可版本化](#原则-8任何业务对象都必须可版本化)
  - [原则 9：带宽由客户端承担，不由 Java](#原则-9带宽由客户端承担不由-java)
...
## 七、2026-04-20 新增原则（项目内部补丁）

以下 3 条原则源自 2026-04-20 用户对"数据持久化"和"可审计性"的强调，不在原 Asana 需求中，但成为本次交付的骨干约束。

### 原则 7：所有长耗时异步状态必须落库

> 任何可能跨越进程生命周期的中间状态（解析中、通知中、学习中）都必须持久化到数据库，不能只在内存里。

**为什么在财务场景下这很重要**:

- 财务工作流跨越多小时。用户可能上传后关闭浏览器、睡一觉、明天继续。
- 后端进程崩溃后重启是常态（OOM、部署、宕机）。内存状态一丢就是用户工作丢失。
- 审计人员会问"为什么这个文件处理了 30 分钟"——只有 DB 记录才能回答，`processing_stage` 列就是这条审计线。
...
- 状态机复杂度增加（20 个 task 状态 + 12 个 file 状态 + 8 个 processing_stage），通过状态推进表和对照文档管理

### 原则 8：任何业务对象都必须可版本化

> 财务数据一旦提交，就成为真相的一部分。但真相会被发现是错的，因此"修订"不是罕见场景，而是必须的一级功能。

**为什么在财务场景下这很重要**:

- 客户更正了 Q1 原始数据；季度末追加 March；发现映射错误——都需要修订，不能强迫用户"从头重新上传"（会产生冲突 + 丢失审计）
- portfolio manager 需要看到"当前是 v2，v1 在 2026-01-15 被替代"的完整链路，否则无法理解财务变更的因果
...
**为什么不做更激进的版本化**（如每次 PATCH /review 都产生版本）:
...
> Java 服务器是编排者，不是代理。大文件流量不应该经过 Java。

**为什么**:

- 旧 multipart 上传：前端 PUT 20MB → Java 接收 20MB → Java 转发 20MB 到 S3 → Java 承担 2×20MB=40MB 流量
- 新 Presigned URL：前端直接 PUT 到 S3，Java 只生成签名 token（几 KB），大幅减少 Java 带宽压力
- 同理文件查看：Java 只生成 Presigned GET URL，不代理下载流

**接受的 trade-off**:

- 增加 S3 CORS 配置复杂度（但一次性配置）
- 前端需要处理 URL 过期 + 自动续签（通过 expiresAt 时间戳 + 4 分钟检查）
- MIME magic bytes 校验延迟到 `POST /upload/complete` 阶段（Java 读 S3 前 2KB 做校验）

**为什么不用 AWS Upload Manager SDK**:

- Upload Manager 设计给大文件（>100MB）分段上传，我们 20MB 单段就够
- 前端用 XHR 直传更简单，onprogress 事件更可靠
- 增加一个 AWS SDK 前端依赖（~200KB），性价比低
