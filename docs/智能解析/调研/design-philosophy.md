...
| **记忆** | `ocr_*` 表前缀隔离 | Chat Agent 来了用 `chat_*`，互不干扰 |
...
```
写入路径（用户确认映射后）：
  1. 写入 ocr_company_mapping_memory → 精确标签到分类的关联
  2. 写入 ocr_mapping_embedding → 向量 embedding 到 pgvector

读取路径（下次上传时）：
  1. Layer 2 查 DB → 精确匹配 / trigram 模糊匹配（零 LLM 调用）
  2. Layer 3 查 pgvector → 注入到 LLM prompt 的 Few-Shot 示例

学习闭环：
  第 1 次上传: "AWS Infrastructure" → LLM 推理 → COGS (MEDIUM) → 用户确认 → 存记忆
  第 2 次上传: "AWS Infrastructure" → DB 精确匹配 → COGS (HIGH) → 零 LLM 调用
  第 3 次上传(不同公司): "Cloud Infra" → pgvector 检索 → 注入 Few-Shot → LLM 推理更准
```
...
| 公司间泄漏 | DB 按 company_id 隔离；向量查询带 company_id 过滤 |
...
|----------|-----------|-------------------|-------------------|
| PostgreSQL | `ocr_*` 表 | `chat_*` 表 | `rag_*` 表 |
| pgvector | 映射 embedding | — | 文档 embedding |
...
| 持续学习 story 太空泛 | P2 | 原则 5 (记忆 > 训练) | 重定义为公司记忆 + 全局向量记忆，不做模型训练 |
...
我们的向量数据是几万条映射 embedding。pgvector 用 HNSW 索引毫秒级响应。我们**真正需要的**是事务一致性：用户确认映射时，业务记录和向量 embedding 在同一个数据库事务中写入。pgvector（同一个 PostgreSQL 实例）天然支持，外部向量数据库需要分布式事务协调。
...
| 不做 | 原因 |
|------|------|
| 通用文档处理平台 | 只处理 LG 的 16 类财务报表。不做发票、合同、税表。范围蔓延会破坏所有优化。 |
