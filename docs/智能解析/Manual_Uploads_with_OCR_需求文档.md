...
- 数据类型：Actuals、Proforma
 - 关键词：
   - “预测数据”的识别信号（满足其中任一条件即视为预测数据）：
    - 表格标题、章节标题、sheet名、文件名或邻近文本中包含以下关键词之一：Forecast（预测​​）、Proforma（预测报表）、Projection（推算）、Budget（预算）或 Plan（计划）。
...
- 右上角处有选项可新增（Upload New Documentation）文件 / 替换（Replace Documentation）与删除（Delete Documentation）已上传文件，若文件下拉处选择的ALL选项，则仅显示新增选项。单个文件时不可删除，删除按钮不展示。
  ...
  - **Unmapped accounts**（位于 Actuals / Proforma Tab 上方,受actual/proforma tab控制）
    ...
    - 数据不完整的源账户：
      ...
      - 缺日期 → 显示"No Date"，可从日历下拉选择器中选择开始月份，选择后该数据自动落到相应月份下，若一个account有多个数据，则选择开始月份后，从左到右第一个数据落到开始月份，后面月份依次累加，数据依次落到月份，直到最后一个数据
        ...
        - 特殊情况补充5：
           - 如果没有获取到年月，而且也没办法按已有逻辑推断年月，而且unmapped也没有任何即存的月份，若同类型下已映射（Mapped）版块内存在月份数据有，那就提取该已映射（Mapped）版块中的第一个有效月份，并将其作为当前 Unmapped 版块的起始/第                  一个月份没有，那就获取当前真实日历月份，并以 “当前日历月份 - 1个月”（即上个月）作为当前 Unmapped 版块的终止/最后一个月份，以此向前倒推初始化时间轴。Profoma 板块以 “当前日历月份 + 1个月（自动公司）”或“当前日历月份（手动公司）”作为当前 Unmapped 版块的第一个月份，以此向后推
...
- 为了确保数据库的一致性并防止货币污染，我们目前的策略是：mapped的结果在最终存入数据库前，系统会自动将冲突页面所有判定为“采用新数据（Mapped）”的指标值，强制按汇率规则转换为该月份 LG 系统现存的对应货币。
  在上述案例中，所有被用户勾选“用 Mapped 替换”的欧元值，都会被系统自动转换成美元后，再入库
    - 存储币种逻辑,按月份为单位,分三种情况:
      - 当月 LG 已有既存币种 → 将提取到的数据转换成该既存币种后存入(convert-to-existing)。
      - 当月无既存币种,且只提取到单一币种 → 直接按提取到的原币种存入,不做转换;该币种即成为当月的既存币种。
      - 当月无既存币种,且提取到多种币种 → 将所有数值转换成公司 Settings 中配置的币种后存入;该币种即成为当月的既存币种。
      - 核心规则:一个月份只能有一种币种;币种一旦确立,后续写入都向它看齐(即回到第 1 种,转成既存币种)。
    - 系统目前只支持6种币种： CAD、EUR、GBP、MXN、NOK、USD 币种范围外的数据正常显示。提交时不报冲突，不写入FI库。页面也无提示
