...
**团队已在台账中登记该问题**（`java/CIOaas-api/docs/待优化项.md`，2026-07-27）：条目原文指出两条路径可能挑出不同组织，并建议"定一个确定性排序（如按 organization.created_at 或 code）"。本调研的结论与之一致，但补充一点：**确定性排序只能消除随机性，不能解决"取到的不是租户那一级"**——租户级别需要 Phase 0 的显式标记列才能解决。
...
### 4.3 已有的对象级授权守卫（正面样板，可作为 Phase 1 的改造基准）
...
**这是全仓仅有的、在请求期判定"这个调用者能不能看这行数据"的实现**，覆盖 3 个端点。它是 Phase 1 要推广到全站的模式，也是最贴合的工作量基准（见 [决策摘要](./executive-summary.md) §4）。
...
即团队已自认：**`seeAllPortfolio` 分支不校验组织归属，且收口需要改动决策链本身**——这正是 Phase 1 中 J-6 的成本来源。
...
| # | 缺口 | 优先级 |
|---|---|---|
| J-1 | `Organization` 无租户标记列，租户不可查询 | P0 |
| J-2 | 三个"取哪一级"实现互不相同且均为无序取一 | P0 |
| J-3 | 39 个接口接受 `organizationId`，0 个校验归属 | P1 |
| J-4 | 2 处反向权限倒挂 | P1 |
| J-5 | 授权链等值不递归，与展示层子树口径错配 | P1 |
| J-6 | 「`company_id` 为空 ⟺ 超管」不变式与租户管理员冲突 | P1 |
| J-7 | 两条 org→company 路径不同步、无租户一致性约束、`findAllByCompanyId` 排序口径与页面树不一致 | P1（排序一条可当下单独修） |
| J-8 | 组织创建/删除/搬家均无权限校验；删除级联严重不全；搬家可造环 | P1 |
| J-9 | 无框架级过滤机制（L3 缺失） | P2 |
| J-10 | 操作审计写入链路已断；无登录日志 | P1（SOC 2 阻塞） |
| J-11 | 生产 `ddl-auto: update` | P2 |
| J-12 | 令牌 TTL 约 300 天、无并发控制、改密不失效 | P1 |
| J-13 | 可逆口令副本 + 无授权的口令回吐接口 | **P-1 阻断** |
| J-14 | 生产 JWT 密钥为默认占位串且配置在 git 中 | **P-1 阻断** |
