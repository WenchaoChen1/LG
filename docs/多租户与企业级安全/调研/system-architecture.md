...
⚠️ 由此推翻一个常见的修复直觉：**不能给 `(company_id, organization_id)` 加"一个公司只能挂一个组织"式的唯一约束**——那会违反业务规则。正确的约束是两条：① 防同一对重复插入的唯一约束；② **租户一致性约束**（一个 company 的全部组织归属必须落在同一租户子树内），后者目前**完全不存在，且需要 Phase 0 的租户标记列才写得出来**。
...
**结论：租户边界当前不是"定在了错误的一级"，而是"每条代码路径定在不同的一级"。** Phase 0 的本质是把这 6 个收敛成 1 个，而不是新增第 7 个。
...
> ⚠️ **"等值不递归"本身不是产品缺陷。** 前端组织树下拉允许用户切换到任意子组织节点浏览其下的 Portfolio 与公司，**按单节点查正是现有交互的设计**。以下两个方向的后果，说的不是浏览场景。
...
**方向二：租户级圈定给不出集合。**
Phase 1 需要的是"某租户下全部公司"这一个整体集合来做强制过滤。等值查询只能给出单节点结果，**要么改成递归展开，要么在应用层自行遍历子树**——这是 Phase 1 中一处确定的改造点，而非现存 bug。
...
| Java 认证 | JWT 仅含 `sub = userId`；租户信息在 Redis 会话，但全仓仅 2 处代码读取 | L1 |
| Java 数据访问 | 无任何框架级过滤（无 MyBatis 拦截器、无 Hibernate Filter、无 SQL 改写、无 MVC 拦截器）；每个 Repository 手写条件 | L1 |
| Python 身份 | 唯一可信来源是 Java 写入的 Redis 会话；全局中间件只验令牌有效性，不判租户 | L1–L2 |
| Python 各模块 | 差异极大：memory / file_registry / 训练样本达 L2；RAG 读路径、LLM 与链路追踪端点为 L0 | L0–L2 |
| 前端 | 门禁数据源为可写 `localStorage`；无统一租户上下文 | 不构成安全边界 |
| 数据库 | 单库单 schema 全租户共享；无 RLS、无角色分权、无分区 | L0 |
...
| 方案 | 隔离强度 | 主要阻碍 | 对现有功能影响 |
|---|---|---|---|
| **A. 每租户一库** | 最强 | 运维成本随租户数线性增长；迁移、备份、监控各乘 N | **破坏跨租户聚合**（对标 benchmark 同侪集、组织级汇总） |
| **B. 每租户一 schema** | 强 | 生产开着 `ddl-auto: update`，表结构由实体自动演进，多 schema 下此路不通；全局字典表如何共享需另设计 | 同上 |
| **C. RLS** | 中强（DB 兜底） | 见 §6.3；**逐项功能改动明细见 §11** | 小 |
| **D. 按租户分区** | 弱（性能手段） | 大表重建；分区键须进主键 | 不解决越权，不在本题范围 |
...
**估算：在应用层 Phase 0–2 完成的基础上，额外 5 人周（初步估算，明细见 §11）。**

### 6.5 结论

**不建议现阶段做数据库级隔离。理由不是工作量，而是顺序。**

数据库级隔离的价值是"应用层漏写条件时兜底"。而 LG 的问题不是漏写——条件基本都写了（L1），问题是条件里那个租户 ID **来自客户端且零校验**，是"我在哪个组织"有 6 个答案，是多归属时"取第一个"的排序口径不统一。**这些不解决，RLS 策略里的"当前租户"根本没有可信的值可填；这些一旦解决，数据库级隔离的绝大部分收益已经拿到。**

同等工作量投入应用层 Phase 0–2，能覆盖全部已识别的越权面；投入数据库层，则必须先把认证链、130+ 张表的列与索引、以及两个库的迁移体系全部重做一遍，做完仍然要回来做 Phase 0–2。
...
> **初步估算：路径 A（P-1 ~ P3 全部）合计 7 人周**，依据是 §10 的逐项功能改动明细（口径「只算功能修改、不含运维」）。
> 下表原按阶段给出的人周数已被上述总量取代，故改为只标相对量级；阶段内的具体分配待排期时确定。

| 阶段 | 目标 | 关键交付 | 相对量级 |
|---|---|---|---|
| **P-1 阻断项** | 身份不可伪造 | 轮换签名密钥与全部凭据、清理 git 历史、下线或收敛口令回吐接口、移除可逆口令副本 | 小 |
| **P0 租户模型** | 让"租户"可查询 | `organization` 租户标记列 + 标定；6 个实现收敛为 1 个可信来源并入认证链；补租户一致性约束 | 小 |
| **P1 服务端强制（L2）** | 客户端不能再决定看谁的数据 | 39 个接口改服务端解析；2 处倒挂修复；授权链改子树展开；Python 作用域收敛；前端统一租户上下文；替换「company_id 为空 ⟺ 超管」不变式 | **最大** |
| **P2 框架级强制（L3）** | 漏写即拒绝 | 统一过滤机制；16 张无租户键表归属方案 | 中 |
| **P3 租户运营** | 可开通、可注销、可管理 | 开通向导（版本化种子）、注销级联、组织级权限管理 UI（多为改造非新建，见 §10.4） | 中 |
| ~~P4 数据库级~~ | ~~纵深防御~~ | **不建议**，见 §6.5 与 §11.5 | ~~另计 5 人周（初步估算）~~ |

企业级安全的现状与差距明细见 §9；其分期与 P0/P1 高度重叠（零信任改造与 P0 实为同一件事），决策级估算见 [决策摘要](./executive-summary.md) §4。
...
|---|---|
| 用户密码哈希 | BCrypt ✅ |
| **用户口令可逆副本**（`user.pwd`） | 🔴 明文直存或 AES-ECB（密钥硬编码），见 §6 与 [证据代码](./code-examples.md) §6 |
...
**密钥管理**：正规路径是 Nacos 配置中心 + `.env`（未提交），但被阻断项 1 绕过——`deploy/*.yml` 四份配置**被 git 跟踪**且含生产凭据。**全仓无 AWS Secrets Manager / SSM / Vault / KMS 调用**（`rag/infrastructure/crypto/__init__.py:4` 只写了"生产应由 KMS 托管"的期望）。Python 侧另有一处硬编码回退令牌（`common/config/lgpi.py:28-31, 41-46`）。
...
**"删除一个租户"的实际成本**：全仓 `ON DELETE CASCADE` 仅 7 处且**无一指向 company 或 organization**；删除路径本身已不可达（见 §7.2）。要真删需人工处理 85+ 张带 `company_id` 的表、10 张仅有 `organization_id` 的表，以及 16 张需 2–3 跳 JOIN 反查的表——其中 `files` 与 `checkpoints` **没有可靠反查键**。
...
| 企业级安全项 | 与多租户分期的关系 |
|---|---|
| **零信任** | **与 P0 是同一件事**——两者的第一步都是"让认证链承载可信身份与租户上下文"。分开排期会重复约 2–3 人周 |
| **审计日志** | 独立，但需在 SOC 2 观察期开始前上线（控制措施须先运行才能收集证据） |
| **GDPR 删除能力** | 依赖 P0/P2——"删除一个租户"需要先有租户归属标记与完整反查链 |
| **SOC 2 访问控制域** | 直接由 P1/P2 满足 |
...
> **初步估算**：依据本清单初估，**路径 A（第 0–2 步 + 租户运营）合计 7 人周**。§8 分期表与决策摘要 §4.2 均已按此更新。

### 10.1 第 0 步：租户模型落地

| # | 改动项 | 端 | 规模 | 备注 |
|---|---|---|---|---|
| 0-1 | `organization` 加租户标记列 | Java + SQL | 1 列 + 1 个迁移文件 | 不能用层级深度推断，必须显式列 |
| 0-2 | 标定租户节点 | 数据 | **1 条 UPDATE** | 当前只有 Golden Section 一个租户 |
| 0-3 | 租户 id 进认证链 | Java | 4 个类：`JwtTokenProvider` / `UserContext` / `UserInfoCache` / `OAuthTokenController` | 签发时写入 + 会话结构扩展 |
| 0-4 | Python 读取契约同步 | Python | `auth_store.AuthUser` 加字段，1 处 | |
| 0-5 | **收敛 6 个"我在哪个组织"实现** | 三端 | Java 3 处（`findOrganizationIdByUserId` / `UserDetailsServiceImpl` / `getCurrentOrganization`，后者有 2 份复制）+ Python 1 处（`company_service` 取 `organizations[0]`）+ 前端 2 处（`firstLeafOrgId`、13 处 `data[0].id`） | 本步的主体工作 |
| 0-6 | 租户一致性约束（DDL + 写入校验） | Java + SQL | 2 个写入点（`CompanyServiceImpl` 的 `update` / `inviteCompany`） | 见 §1.2：不可加"一公司一组织"式唯一约束 |
| 0-7 | `findAllByCompanyId` 补 `ORDER BY created_at` | Java | **1 行** | 与页面树口径对齐，见 §1.2 |
...
| # | 改动项 | 端 | 规模 |
|---|---|---|---|
| 1-1 | 授权链改子树展开 | Java | `findVisibleCompanies` 1 条 SQL + 调用方；递归 CTE `findByTreeInId` 已存在可直接用 |
| 1-2 | **替换「`company_id` 为空 ⟺ 超管」不变式** | Java + Python | **4 个模块**：Java 登录链、chatbot 端类型判定、rag `_require_admin`、financial_extract ACL。**本步风险最高的一项** |
| 1-3 | Python 组织作用域收敛 | Python | 7 个模块：memory / file_registry / chatbot manage / financial_extract / tracing / llm / rag |
| 1-4 | Python `list_org_companies` 链改子树 | Python | 1 条链 |
| 1-5 | rag 读路径补归属 | Python | 4 个端点（`/spaces`、`/spaces/{id}/chunks`、`/entries/{id}/chunks`、`/recall`） |
| 1-6 | **前端统一租户上下文** | 前端 | 新建 1 个 Context/model + 替换 **65 处 `inviteDto` + 63 处 `getQueryString('id')`**（机械但量大） |
| 1-7 | 前端组织树下拉收敛 | 前端 | 17 个文件中复制的 TreeSelect + 5 份重复 service |
| 1-8 | 平台用户管理放开查询维度 | 前端 + Java | `UserManagePage` 现写死 `roleTypes: 1`（只查超管），需支持按组织查全部用户 |

### 10.3 第 2 步：框架级强制

| # | 改动项 | 端 | 规模 |
|---|---|---|---|
| 2-1 | Java 统一过滤机制（Hibernate Filter 或拦截器） | Java | 1 套机制 + 全量 Repository 回归 |
| 2-2 | Python 统一依赖 | Python | 1 套机制 |
| 2-3 | 补 16 张无租户键表 | 三端 + SQL | 14 张可反查；**`files` 与 `checkpoints` 无可靠反查键，需先设计归属方案** |
...
- 第 0 步 **全部**（0-1 ~ 0-7）
- 第 1 步的 **10.2.1（39 个接口）+ 1-1 / 1-2 / 1-3 / 1-6**
- 租户运营的 **开通向导（脚本版即可）+ 种子改版本化模板 + 组织搬动权限**
- 附录性质的 **两条阻断项**（见 [决策摘要](./executive-summary.md) 附录 A）

**可延后**：第 2 步全部（框架级强制是"漏写也安全"的加固，不是接客户的前提）、1-4 / 1-5 / 1-7 / 1-8、以及 §10.4 中标注"可延后"的全部条目。

> 若第二家客户的日常组织/成员/权限维护由**内部代运营**，则 §10.4 中"成员总览 / 邀请生命周期 / 角色批量分配 / 套餐席位"四项可整体延后——**这是产品侧的一个选择，直接决定这批工作要不要做**。

---

## 11. 路径 B（数据库层隔离）改造项清单

> **前置**：本清单**全部建立在路径 A 已完成的基础上**。B 不能替代 A——RLS 策略里的"当前租户"必须由 A 的第 0 步提供可信值，否则无值可填（理由见 §6.5）。
> **方案取向**：四种方案中只有 **RLS** 成本可控（分库/分 schema 会破坏跨租户聚合功能，分区不解决越权）。本清单按 RLS 路线展开。
> **口径**：同 §10——只列功能改动，不含运维。
...
| # | 改动项 | 规模（实测） | 说明 |
|---|---|---|---|
| B-1 | 引入反规范化 `tenant_id` 列 | Java 侧 **98 张**已有归属列的表 + **16 张**无归属键的表；Python 侧 **33 张** ORM 表 | 让 RLS 谓词从"递归子树集合判断"退回"等值判断"，**这是 RLS 可行的唯一前提** |
| B-2 | 存量回填 | **单租户下是一次全量 UPDATE** | 当前唯一租户，回填成本≈0；有第二家客户后成本剧增 |
| B-3 | 补索引 | 每张表 1 个；且测试库有 **288 个 `*_id` 列不是任何索引首列**；向量库 3 张分段表归属列**零索引** | 大表须 `CREATE INDEX CONCURRENTLY` |
| B-4 | 归属字段命名统一 | **4 类别名**：`ref_company_id`（2 张）/ `company_ref`（1 张）/ `main_company_id`（1 张）/ `portfolio_id` vs `company_group_id` | 不统一则任何批量脚本都会漏表 |
| B-5 | `files` 与 `checkpoints` 归属方案设计 | 2 组（`checkpoints` 实为 3 张表） | **无可靠反查键**，须先设计再实施。`checkpoints` 由 LangGraph 在启动时 `PostgresSaver.setup()` 自建（`source/bootstrap/lifespan.py:119-121`），**不受版本化迁移管控**——性质类似 Java 的 `ddl-auto`，但范围仅 3 张表 |
| B-6 | 全局字典表豁免清单 | 待梳理（`currency` / `dictionary` / `menu` / `role` / 对标基准数据等） | **漏排会把跨租户共享数据全部拦掉**，属高风险项 |

### 11.2 数据库配置改造

| # | 改动项 | 规模 | 说明 |
|---|---|---|---|
| B-7 | **关闭生产 `ddl-auto: update`** | **仅 Java 侧**：3 个环境配置（`cioaas-web.yml` / `-staging` / `-prod` 的 `:43`）+ 把现有全部表结构收进版本化迁移 | Java 表结构现由 JPA 实体自动演进，与 RLS 方案**根本冲突**。**Python 侧无此问题**——已走版本化迁移（`sql/migrations/`），`create_all` 不在生产路径调用 |
| B-8 | 拆分数据库角色 | 现为**所有服务共用一个超级账号** | **超级账号默认绕过 RLS**，不拆等于策略完全失效；或全表加 `FORCE ROW LEVEL SECURITY` |
| B-9 | 建 RLS 策略 | 约 **114 张（Java）+ 33 张（Python）** 表，每张 2 条语句（`ENABLE` + `CREATE POLICY`） | 机械但量大 |
| B-10 | 两个物理库各做一遍 | 业务库 + RAG 向量库 | 向量库另有 HNSW 索引与列过滤组合的性能问题 |

### 11.3 应用层配套

| # | 改动项 | 规模 | 说明 |
|---|---|---|---|
| B-11 | 连接层注入租户上下文 | Java（Hikari 取连接后 / AOP）+ Python（SQLAlchemy session 事件）各 1 处 | 每个连接或事务执行 `SET app.tenant_id` |
| B-12 | **无用户上下文的执行路径**改造 | 定时任务、SQS 消费者、迁移脚本、后台作业 | 这些没有登录身份，需要系统级角色或显式设租户，**容易被漏掉且出问题时很隐蔽** |
| B-13 | 跨租户聚合功能的豁免机制 | 对标 benchmark 同侪集、组织级汇总等 | RLS 会把这些**正常业务功能一并拦掉**，需专用角色或策略豁免 |

### 11.4 验证与保障

| # | 改动项 | 说明 |
|---|---|---|
| B-14 | 全量接口回归 | RLS 是行级拦截，影响面是**全部读写路径** |
| B-15 | 性能验证 | RLS 对每行做判定；向量库"HNSW + 列过滤"会退化为先扫再排序 |
| B-16 | 运维与排障预案 | 策略生效后，跨租户运维查询、线上排障、数据修复都需要新的操作路径 |

### 11.5 判断

B 的**机械工作量**（B-1/B-2/B-3/B-9）在单租户现状下并不算大——回填是一次 UPDATE，建策略是批量脚本。**真正的成本与风险集中在四处**：

1. **B-7 关闭 `ddl-auto`** —— 要先把现有全部表结构收进版本化迁移，这本身是一次独立的工程
2. **B-8 拆数据库角色** —— 不拆则 RLS 形同虚设，拆则牵动全部服务的连接配置
3. **B-6 + B-13 豁免清单** —— 漏排会打断正常业务功能，且故障表现为"数据莫名其妙查不到"
4. **B-12 无用户上下文的路径** —— 定时任务与消息消费者最容易被漏

**结论不变，但理由要说准**：B 初步估算为 **5 人周**，**比路径 A（7 人周）还小**——所以"不建议做 B"**与成本无关**。三条真实理由：

1. **B 依赖 A，不能替代 A。** RLS 策略里的"当前租户"必须由 A 的第 0 步提供；A 没做，B 的策略无值可填（§6.5）
2. **收益重叠。** A 完成后达到第 3 档（漏写即拒绝），B 只额外覆盖"应用被完全绕过"这一种情形——而那种情形下攻击者已持有数据库凭据，B-8 若未彻底拆角色，RLS 同样被绕过
3. **风险不在工作量而在副作用。** 上面四项（B-6/B-7/B-8/B-12/B-13）出问题时，表现是**正常业务功能突然查不到数据**，排查成本远高于开发成本
