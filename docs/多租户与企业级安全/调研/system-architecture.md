...
organization                 表 organization，PK = id，pid 自引用（递归树，深度不固定）
    ...
    │
    ├─ **任意节点即一个租户**；父节点管理其子租户（整棵子树）
    │     生产现有 5 个：1 个 Golden Section（真实客户）+ 4 个测试组织
    │     租户键 = `organization_id`（管理端）、`company_id`（客户端）——**无需新增标识列**
    │
...
> **业务规则（2026-09-07 修订）**：一个 company **可以同时挂在多个组织下**，这是**正常业务形态**。既然每个组织都是租户，这就意味着一家企业会被多个租户共享；真正需要的约束是**共享范围不得越出某个上级租户的子树**（多归属必须落在同一棵子树内）。**不可加"一公司一组织"式唯一约束。**
...
> ⚠ **"等值不递归"在浏览场景下不是缺陷。** 前端组织树下拉允许用户切换到任意子组织节点浏览其下的 Portfolio 与公司，**按单节点查正是现有交互的设计**。
>
> 但锚定修订后多了一个新场景：**父租户要看自己名下（含全部子租户）的全量数据**。这个场景现在**做不到**——等值查询只能给出单节点结果。以下三个方向的后果，说的不是浏览场景。
...
## 4. 核心问题三：40 个接口接受 organizationId，0 个校验归属

全量扫描 50 个 Controller 的结果：

- 服务端**真正读取** `organizationId` 的端点：**40 个**
...
| 前置条件 | 现状 | 差距 |
|---|---|---|
| ① 应用能提供**可信**的租户上下文 | 键在表上，但 JWT 无 `organization_id` claim，实际取值来自客户端参数 | **致命**。策略里的"当前租户"没有可信值可填 |
| ② 每张表都有**可直接等值判定**的租户列 | Java 侧 151 个 JPA 实体中 85 个有 `company_id`、21 个有 `organization_id`、13 张二者皆无（其余 40 张为全局字典/权限元数据，合理无归属）。但**无一张带可直接用作策略谓词的去规范化列** | 子租户使谓词成为子树成员判定，需新增打戳列 + 回填 |
...
5. 85+ 张表加列 + 回填（5 个租户、公司数量很小，回填是一次批量 UPDATE）+ 建索引
...
### 10.1 第 1 步：让租户归属变成可信的

| # | 改动项 | 端 | 规模 | 备注 |
|---|---|---|---|---|
| ~~1-1~~ | ~~`organization` 加租户标记列~~ | — | **不需要** | 锚定修订后作废：`organization_id` 本身就是租户键 |
...
### 10.5 最小可行范围（目标：能安全接外部租户）

若只求"安全接入第一个真实外部租户"，**必须做**的是：

- 第 1 步 **全部**（1-1 ~ 1-7）
- 第 2 步的 **10.2.1（40 个接口）+ 2-1 / 2-2 / 2-3 / 2-6**
...
> 若新租户的日常组织/成员/权限维护由**内部代运营**，则 §10.4 中"成员总览 / 邀请生命周期 / 角色批量分配 / 套餐席位"四项可整体延后——**这是产品侧的一个选择，直接决定这批工作要不要做**。
...
| 4-2 | 存量回填 | **5 个租户、公司数量很小**，回填仍是一次批量 UPDATE | 回填成本≈ 0；但打戳列的值需按子树推导，不是直接拷 `organization_id` |
...
行级策略的**机械工作量**（4-1/4-2/4-3/4-9）在当前数据量下并不算大——回填是一次 UPDATE，建策略是批量脚本。**真正的成本与风险集中在四处**：
...
> 三个问题的共同点：**都不影响"能不能安全接外部租户"**（它们是配置的产品形态，不是隔离缺口），但 Q-2 若选租户级会新增两张表的改造，需在排期时并入第 1 步。
