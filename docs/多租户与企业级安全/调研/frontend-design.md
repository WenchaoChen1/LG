...
| # | 缺口 | 优先级 |
|---|---|---|
| F-1 | 无统一租户上下文，公司/组织 ID 散落 4 处（`inviteDto` 65 处 + `getQueryString('id')` 63 处） | 第 1 步 |
| F-2 | 切换组织依赖后端回传口令 → 应改服务端令牌交换 | **优先项（阻断）（与 Java 同一问题）** |
| F-3 | 三处 URL query 可篡改 `organizationId` 且绕过前端白名单 | 第 1 步（后端补校验后自动失效） |
| F-4 | 权限门禁数据源为可写 localStorage，前端不构成安全边界 | 第 1 步（依赖后端补齐） |
| F-5 | 层级假设互相矛盾（取根 vs 取叶），且中间层可选 | 第 0 步（随第 0 步收敛） |
| F-6 | 组织树下拉代码复制到 17 个文件、service 重复 5 份 | 第 2 步 |
| F-7 | 端类型两套口径并存 | 第 2 步 |
| F-8 | 多租户运营 UI 缺 12 项（见 §9） | 第 3 步 |
