...
| **v4.67（本版）** | **2026-09-24** | **需求方 2026-09-24 第二次：A4 管理端无提交也不渲染元数据栏（同版追加：A3 跟改）** | **纯前端两处渲染门槛（A4 + A3），后端 0 改动、接口契约不变。**A4 全维 Score Details 页**管理端**所选端（`GSV` / `Founder`）在该维该期次**无提交**（`portals[].submission == null`）时，**不再渲染** `PERIOD` / `SUBMITTED BY` / `ROLE` / `SUBMITTED AT` 四格元数据栏，卡内只留空态 `No {端} submission for this period.` —— 推翻 2026-09-20「管理端恒显示，无提交即占位符（`PERIOD` 回退当前期次、其余三格 `—`）」在 A4 上的口径（v4.66 §0.39-M4 当时维持不变，本版推翻）。至此 **A4 两端同一条规则：该维该端无提交 ⇒ 不渲染元数据栏**。前端：`DimensionQuestionsCard` 显示条件由 `portal \|\| meta` 改为 `meta`，`fallbackPeriod` prop 删除。~~**A3 维度详情页不变**（管理端仍恒显示，本次只改 A4）~~ → **同版追加（需求方 2026-09-24）：A3 维度详情页同步跟改** —— 该端该期次无提交 ⇒ 元数据栏整条不渲染，只留 `No {端} submission for this period.`；`DimensionPage` 渲染条件由 `detail && (isAdmin \|\| detail.submission)` 改为 `detail?.submission`，`SubmissionMetaBar` 的 `submission` 入参收为非空、`fallbackPeriod` 入参删除。**至此 A3 / A4 两页、两端同一条规则：无提交 ⇒ 不渲染元数据栏**。详见 §0.40（N4） |
