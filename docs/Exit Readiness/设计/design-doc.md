...
>
> 阶段：④ 设计 | 版本：**v4.22** | 日期：2026-09-10 | 范围：ERL Card（含 Gap 区块）/ 维度详情 / 全维 Score Details（**双端可达**）/ 双端填报（**Yes/No 逐级解锁 + 维度级提交**）/ 题库配置（**`Question Library` / `Dimension Configuration` 两个顶层 Tab + 题库版本历史页**）/ 基准 / Goldie 差距分析（**已回归 V1，含 Share**）/ 组合层 ERL Tab
...
| **D1 卡一：Latest by Dimension** | 卡头 `Latest by Dimension · {period}`；表格三列 `DIMENSION` / ~~`BENCHMARKIT`~~ / ~~`TOP GSV QUARTILE`~~ → **2026-09-09 按原型改名（§0.24-Z4）**：**`EXTERNAL BENCHMARKS`** / **`TOP GSV QUARTILE`**（~~`(1-9)`~~ 后缀 2026-09-10 去掉，§0.26-Z2；`BenchmarkDimensionTable` 是 A4 末位基准 Tab 与 D1 两张卡共用组件，同一个概念不该在两页两个叫法，故 **D1 这两处一起改**）；⚠️ **D1 无 `Average` 行** —— ~~表尾加权 `Average` 只在传了非空 `weights` 时渲染，基准列表出参里没有权重~~ → **v4.22 起该行连 A4 都没有了**（§0.26-Z1），三处一律只有明细行。~~五行~~ → **v4.4：一维一行，行数按配置维度动态**（§0.10-D1）；维度列 `**FRL** Financial Readiness`（缩写加粗 + 全称次级色）；分值格式 `6.5/9`，`/9` 次级字号 | 原型（§0.3-4） |
