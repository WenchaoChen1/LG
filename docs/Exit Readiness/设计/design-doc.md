...
>
> 阶段：④ 设计 | 版本：**v4.37** | 日期：2026-09-15 | 范围：ERL Card（含 Gap 区块）/ 维度详情 / 全维 Score Details（**双端可达**）/ 双端填报（**Yes/No 逐级解锁 + 维度级提交**）/ 题库配置（**`Question Library` / `Dimension Configuration` 两个顶层 Tab + 题库版本历史页**）/ 基准 / Goldie 差距分析（**已回归 V1，含 Share**）/ 组合层 ERL Tab
...
- **v4.0**：各维度列的值是 **level 分（0–9 整数）**；`overallScore` 是**该公司当前 `status = 'Active'` 维度集合上的等权平均**（**2026-09-15 起不再加权**，§7.1）—— 分子 = Σ(各维度分，该期次未提交的维度按 `0`)，分母 = 该维度集合的**个数**，结果一位小数。⚠️ **跨组织同屏时维度集合可能不同** —— F2 只列当前用户有权访问的公司，正常场景同属一个组织；若出现跨组织，综合分**各按各自组织的维度集合算**（分母因此可能不同），不做任何跨公司归一化，那会改变每家公司自己的分数。
...
| **综合分（v4.0 换底 · v4.4 换权重来源 · 2026-09-15 改等权）** | **Σ(各维度分) ÷ 维度个数**，保留 1 位小数，以 `X/9` 展示（需求方 2026-09-15）。<br>**分母 = 该组织当前 `status = 'Active'` 的维度个数，不是已填写的维度个数**：配了 5 维只填了 2 维（得分 6、4）⇒ `(6+4)/5 = 2.0`，不是 `(6+4)/2 = 5.0`；配了 6 维分母就是 6。<br>**未填写的维度按 `0` 计入分子** —— 没做的评估就是没分，会把综合分拉低。这与 v4.4 的口径**正好相反**（原先是「剔除该维、其余权重按比例归一化」，§0.9-20），该条已被本次裁决作废。<br>~~**Σ(维度分 × 该维权重%)**，权重取 `erl_dimension_config` 中该组织 `Active` 行的当前 `weight`~~ → **2026-09-15 作废**：综合分**不再按权重加权**。`erl_dimension_config.weight` 配置与接口 24 的「合计必须 = 100」校验**保留不动**，但综合分不再读它（当前唯一消费者是 Goldie 差距分析的入参，§6.6）。<br>**维度集合**：仍取当前 **`status = 'Active'`** 的维度集合（`Inactive` 行不参与、也不占分母位），**个数不固定、不假设五维**（§0.10-D1）。维度停用后，它在历史期次的已提交分数随之退出分子分母两侧。<br>**唯一的空态**：该公司该期次**一维都没填** ⇒ 综合分 `null`（前端灰底 `—`），**不按 `0.0` 算**。⚠️ 这条必须留着 —— 否则「零提交」与「填了、level 1 就没过」的合法 `0` 分在界面上分不出来（见本节末段 `0` vs `null`） | **2026-09-15 需求方定档**（此前为 PRD §3.1 / §3.8 + v4.4 §0.10-R2 / D1 的加权口径） |
...
综合分（2026-09-15 起等权，分母 = Active 维度个数；下列均设该组织配了 5 个维度
        FRL / PRL / BERL / RRL / TRL）

① 五维全填，维度分 2 / 5 / 7 / 4 / 1
              = (2+5+7+4+1) / 5 = 19/5 = 3.8 → 展示 4/9（Founder Era）

② **只填了 2 维**（需求方给的算例）：FRL = 6、PRL = 4，其余三维未填
              = (6+4+0+0+0) / 5 = 10/5 = 2.0 → 展示 2/9（Stage 2 · Founder Era）
                ⚠️ 不是 (6+4)/2 = 5.0 —— 分母恒为配置的维度个数

③ 组织配了 **6** 个维度、只填了 3 维（8 / 7 / 6）
              = (8+7+6) / 6 = 21/6 = 3.5 → 展示 4/9（Era 仍按 3.5 判 Founder，不按取整后的 4）

④ 只填 1 维且得 0 分：= 0/5 = 0.0 → 展示 0/9，红底 + `Not yet Stage 1`
⑤ 一维都没填：= null（**不是 0.0**）→ 灰底 `—`，Stage / Era 一并空
```

> **示例只是示例，维度数不固定**：上式取五维仅因原型如此，**实现不得写死 5**（§0.10-D1）。
> 真正的算法是「遍历当前 `status = 'Active'` 的全部维度，Σ(维度分，未填按 0) ÷ 维度个数」——
> 维度个数、代码、排序全部来自 `erl_dimension_config`（`organization_id` + `status='Active'`，按 `sort_order`；**2026-09-08**：原 `erl_company_period_config → erl_dimension_config_item` 两张表已删，§7.11），
> 而**不是**枚举 `ErlDimensionEnum` 或前端 `constants.ts` 的静态映射（两者均已删除）。
> **唯一实现**：`ErlScoreCalculator#overallAverage`（Java），前端不再自算（原 `weightedOverallScore` 已删）。
> ~~`null` 归一化的分母是「剩余维度的权重之和」~~ → **2026-09-15 作废**：不再归一化，未填维度按 0 占分母位。
