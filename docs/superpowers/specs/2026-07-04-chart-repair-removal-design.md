...
处置建议：**不在前端复刻后端 format 校验**（两份规则违反单点原则、维护成本高于收益）；validate_chart 预检 + 数据红线是第一道闸，此类 fence 本就极低频。上线后若观察到降级/复活频率高，再议（届时优先恢复修复回路而非双份校验，见 §10）。
