...
- legend的显隐不影响总分的计算，总分只受filter影响
- Internal peer时：P0, P25, P50, P75, P100及对应的实际值；外部平台时：P25, P50, P75及其对应实际值，若无数据则显示N/A。
- Actual字体是白色，Committed Forecast字体是紫色，System Generated Forecast字体是加图标的紫色。
- 提示：external benchmark因为没有录入预测数据，所以没有预测数据的基准点位信息，只会显示一行白色数据，没有紫色数据
后面所有公司以此类推，全部显示
```
##### 数据缺失处理
