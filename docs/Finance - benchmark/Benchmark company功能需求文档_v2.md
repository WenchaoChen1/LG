...
用户进入Benchmark页面							
    ↓							
查看默认参数配置							
    ├─ 时间范围：Snapshot视图时：the last closed month数据 Trend视图时：the last closed month往前数6个月
        背景补充：
        the last closed month定义： Financial Statements Settings中为Manual的公司，the last closed month是Financia Entry表中最后一个有Actuals数据的月份；
        Financial Statements Settings中为Automatic的公司，the last clsoed month以15号为界限，如果系统服务器时间过了15号，就是上个月；
        如果系统服务器时间没过15号，就是上上个月。
...
选择筛选条件							
    ...
    ├─ DATA：选择数据类型（Actuals、Committed Forecast、System Generated Forecast可多选）					
...
- 若该公司状态为"Exited" 或 "Shut Down"，则显示空页面，展示“Benchmarking data is unavailable because this company is not active in Looking Glass	”		
...
| **FILTER** | ALL, GROWTH, EFFICIENCY, MARGINS, CAPITAL | 多选（ALL为总和） | 蓝色点亮 | ALL与其他选项互斥，选中ALL后如果选择其他选项，ALL就自动取消勾选，除All外其他选项可同时选中；决定显示哪个板块的数据 |							
| **DATA** | Actuals, Committed Forecast, System Generated Forecast | 多选 | 黄色点亮 | Actuals：历史实际数据，数据来源于系统现有的Normalization Tracing模块；Committed/System Generated Forecast：预测数据,数据来自于系统现有的Normalization Tracing模块；多选时显示多个维度 |						
| **BENCHMARK** | Internal Peers, KeyBanc, High Alpha, Benchmarkit.ai | 多选 | 黄色点亮 | 基准数据来源；最多4个；与DATA组合形成最多12个维度 |						
...
#### 4.1.3 日历选择区域							
**位置**：筛选卡片下方或右侧							
**默认值**：Snapshot视图时(单个日历选择器）：the last closed month数据
        背景补充：
        the last closed month定义： Financial Statements Settings中为Manual的公司，the last clsoed month是Financia Entry表中最后一个有Actuals数据的月份；
        Financial Statements Settings中为Automatic的公司，the last clsoed month以15号为界限，如果系统服务器时间过了15号，就是上个月（前提是Financial Entry表中上个月有Actuals数据，若没有就继续往历史月份找，找到有actuals数据的月份为止）；
        如果系统服务器时间没过15号，就是上上个月（前提是前提是Financial Entry表中上上个月有Actuals数据，若没有就继续往历史月份找，找到有actuals数据的月份为止）。	
        Trend视图时（两个日历选择器代表起止月份，默认选择六个月）：默认显示 closed month 及其前 5 个月，共 6 个月数据（含首尾月）
        两种视图切换时，日历选择器要切换				
**功能**：用户可选择不同月份查看对标数据							
...
2. **进度条（Bar）**							
   ...
   - 颜色规则（根据 Overall Score 的百分位 n）：							
     ...
     在第三方基准排位时，如果百分位为超范围显示，则进度条颜色按其相邻的基准点位填充（点位为P0， P25， P50， P75， P100）。比如＞P75，则进度条颜色按P100显示；比如＜P25，则进度条颜色按P0显示。（详见6.1.2）							
...
3. **进度条点位（Benchmark Points）**							
   ...
   - **特殊处理**：若多个点位完全重合，打包展示所有重合点位信息，点位展示顺序按下面Data-Benchmark展示排序来展示。比如Actuals - Internal Peers和Committed Forecasts - Internal Peers重合，那点位就显示Actuals - Internal Peers（颜色和形状），悬浮的时候打包展示,排列顺序见4.2.1.6					
...
4. **右上角信息提示**							
   ...
   - **特殊提示**（当存在特殊计算时）：							
     ...
     "Includes estimated percentiles (interpolated values used/boundary values used/interpolated values & boundary values used)"（详见6.1.2）					
...
1. Revenue & Growth（收入增长）- 对应 FILTER 的 GROWTH-指标ARR Growth Rate							
2. Profitability & Efficiency（盈利能力&效率）- 对应 FILTER 的 EFFICIENCY-指标	Gross Margin						
3. Burn & Runway（现金消耗&跑道）- 对应 FILTER 的 MARGINS-指标	Monthly Net Burn Rate, Monthly Runway						
4. Capital Efficiency（资本效率）- 对应 FILTER 的 CAPITAL-指标Rule of 40, Sales Efficiency Ratio							
...
2. **板块进度条**							
   - **注意：各板块的进度条不显示维度点位，仅显示该板块下所有指标的平均百分位**							
   - 颜色根据板块百分位显示							
							
3. **单个指标展示**（每个板块下的指标）							
   - **指标名称**：对应指标英文名	
   -  Actual、Committed Forecast、 System Generated Forecast的实际值，实际值均取于Normalization Tracing模块的对应值，确保两个页面的显示值是一样的					
   - **分布条**：							
     - 形式：水平彩色条形图							
...
"     -各指标名称悬停内容：固定显示LG的指标名称和对应LG公式，动态显示当前页面选择的各个Benchmark对应的指标名称，指标公式，样本分组信息，公式信心标签，对标平台名称和版本。Benchmark信息取自LG平台已完成功能“Benchmark Entry”,  分别对应 Metric Name，Definition, Segment Type, Segment Value (Segment Type, Segment Value组合显示)，Best Guess, Platform, Edition (显示格式为Platform - Edition)。						
   ...
   - **百分位数值**：							
     - 精确百分位：P45（例如）							
     - 估算百分位：~P64（使用了线性插值，详见6.1.2.2）							
     - 超范围标记：>P75 或 < P25，详见6.1.2.1						
...
4. **板块展开/收起**							
   - 点击板块标题可展开/收起该板块							
   - 默认初试化卡片展开状态，指标是收起状态
   - 板块默认展开：显示板块的名称，百分位和进度条；板块内每个指标默认展示一个DATA-BENCHMARK分布条，如果一个指标存在多个分布条，那就显示Show All按钮，点击显示全部分布条，按钮变为Hide。							
   - 板块收起，只显示板块的名称，百分位和进度条							
							
5. **Snapshot 视图中的 FILTER 行为**：							
   - 选择 FILTER=GROWTH 时，页面仅显示 Revenue & Growth 板块的指标							
   - 选择 FILTER=ALL 时，显示全部 4 个板块							
   - 其他 FILTER 选项同理		

6. **Data-Benchmark展示排序**（trend模式下折线图图例的展示也是按这个排序；雷达图的图例也是按这个排序）：
...
- 若月份对应的指标百分位为NA的时候，这个指标百分位的折线点落到P0上，悬浮tooltip的时候展示的百分位显示为NA。							
...
2. **每个板块下的指标折线图**							
   ...
   - 如果百分位为超范围显示，则折线纵坐标按其相邻的基准点位填充（点位为P0， P25， P50， P75， P100）。比如＞P75，则折线点纵坐标落在P100；比如＜P25，则折线点纵坐标落在P0。（详见6.1.2）												
...
5. **计算方法** 
- Overall score: 四个板块中有百分位值的板块的平均值。几个板块有分数就除以几，四舍五入取整
- Bar上每个点的位置：
 - 卡片单个指标时：颜色代表维度（如actual-internal peer），该颜色所对应维度所有月份的平均值四舍五入取整。
 - 卡片两个指标时：颜色代表维度（如actual-internal peer），分别计算两个指标同一颜色的所有月份的平均值四舍五入取整，后两个数值再取平均值四舍五入取整
- 板块百分位：Bar上所有点位的平均值，四舍五入取整 				
							
---							
							
### 4.3 Metrics Summary 雷达图							
							
**位置**：页面底部（Snapshot 和 Trend 视图中都会显示）							
**组成部分**：							
							
1. **雷达图结构**							
   - 中心点：P0（表示最低百分位）							
   - 同心六边形：P25, P50, P75, P100						
   - 角度数：6 个（对应 6 个关键指标）							
   - 线条数：最多 12 条（根据 DATA×BENCHMARK 组合）							
   - 图表类型：固定六边形雷达图（6个指标维度）							
- Snapshot 模式： 数据范围： 单月
   - 计算方式： 每个角的值 = 选定月份数据的百分位
   - 雷达图加图例，方便用户决定网的显隐				   
- Trend 模式： 
   - 数据范围： 用户选择的连续月份
   -计算方式：同一维度所有月份百分位算术平均值							
...
3. **交互**							
   - 鼠标悬浮在指标轴线上时，显示该指标所有 Data-Benchmark 组合的百分位详情,悬浮指标名称							
     - Tooltip内容：							
     - 数据类型（DATA）							
     - 基准来源（BENCHMARK）							
     - 百分位数值（P45或~P64或>P75）
     - 若某个类别（例如“增长”）被取消选中，其对应的指标（即“ARR 增长率”）将显示为灰色。自动将该未选中指标的数值降至P0。							
...
公式：ARR Growth Rate_t = (ARR_t - ARR_(t-1)) / ARR_(t-1)	；若ARR_(t-1) 是N/A,则ARR Growth Rate_t =0; 若ARR_(t-1)=0,则ARR Growth Rate_t =0	
说明：衡量公司的收入增速							
...
说明：衡量公司每月实际消耗的现金							
方向：数值越大越好（正值 = 净收入超出 R&D，盈利能力强）。升序排列，数值越大百分位越高。												
...
特殊情况： Monthly Runway显示为“N/A”的情形：当在非“N/A”月份中，“月度现金跑道”数值显示为“N/A”时：
Top rank（情景：Cash ≥ 0 且Monthly Net Burn Rate ≥ 0）：此类公司处于盈利状态，且拥有正向的现金储备。鉴于它们并未在“消耗”其现金跑道，尽管数值显示为“N/A”，它们仍代表着最高的层级。
Bottom Rank（情景：Cash < 0 且Monthly Net Burn Rate < 0；或 Cash < 0 且Monthly Net Burn Rate = 0；或 Cash = 0 且Monthly Net Burn Rate < 0）：此类公司已陷入资不抵债的境地，且仍在持续亏损。它们代表着最高的风险，因此应被排在最低的位置。
...
4. **特殊情况**							
   ...
   - 若部分同行公司指标值相同：使用竞争排名，分配不同的百分位							
   - 若所有同行公司的指标值完全相同：不显示百分位，不计入metric summary和Overall Score							
  							
							
"#### 6.1.2 外部平台基准（KeyBanc, High Alpha, Benchmarkit.ai）"							
...
"   -线性插入举例：~P62.556.  ~P=P_low + (P_high - P_low) * (d - d_low) / (d_high - d_low)； P为百分位，d为实际数值 ; (例如P50(P_low)=30%(d_low),P75(P_high)=40%(d_low),d=35%;计算~P=50+(75-50)*(35-30)/(40-30)=62.5)(例如P50=30%,P75=40%,d=35%;计算~P=50+(75-50)*(35-30)/(40-30)=62.5)"							
3. **超范围处理**：							
...
5. **外部平台回退规则**
   - 当外部平台所有可用的基准报告中均未找到选定年份的基准数据时，系统将默认使用“最近一份可用基准报告”中的“最近基准数据”。
     首先尝试在“最新一份基准报告”中查找与选定年份完全匹配的数据，随后向后追溯（即查找更早的）基准数据集；若仍未找到匹配数据，则应用通用回退机制，采用最新的基准数据 				
...
#### 6.2.1 
 ...
 - 若无绑定公司，或绑定同行组数据无效，则系统按照匹配规则进行自动匹配，匹配规则见6.2.2。若系统匹配也无有效同行数据，则回退到平台基准，显示Peer Fallback 提示
...
- 用户选择 DATA 类型时（如 Committed Forecast），系统在计算百分位时：							
  ...
  - 不同维度可能有不同数量的有效同行（正常）							
"  -若用户选择Committed Forecast或System Generated Forecast,并选择KeyBanc或High Alpha或Benchmarkit.ai基准时，计算时要用预测数据与已有基准数据计算百分位"								
							
### 6.3 各百分位计算总结							
							
#### 6.3.1 类别百分位（Category Score / 板块百分位）							
							
板块百分位= 该板块所有有效指标的百分位数的算术平均							
							
示例：							
  Burn & Runway 板块包含 2 个指标：							
    - Monthly Net Burn Rate: P55							
    - Monthly Runway: P45							
  Burn & Runway 百分位 = (55 + 45) / 2 = P50							
							
							
#### 6.3.2 Overall Score						
							
Overall Score = 四个板块有数据的百分位的算术平均值，受filter影响，若该板块未N/A或未被勾选，则不计入计算。					
							
公式：							
Overall = (Revenue & Growth + Profitability & Efficiency + Burn & Runway + Capital Efficiency) / 4							
							
							
#### 6.3.3 维度分数（Dimension Score）							
...
### 7.4 指标相同的情况							
**现象1**：同行群组内多家公司的（但非全部公司）同一指标相同					
**处理**：							
使用竞争排名，分配不同的百分位   						

**现象2**：同行群组内全部公司的同一指标相同              
**处理**：                    
 不显示百分位，不计入metric summary和Overall Score						
							
### 7.5 百分位超范围情况							
**现象**：公司指标值超出基准数据范围							
							
**处理**：							
- 高于最高值：显示 > P75，汇总计算使用 P100						
- 低于最低值：显示 < P25，汇总计算使用 P0						
...
| 触发条件 | 显示内容 | 示例 |							
|---------|--------|------|							
| 鼠标悬停进度条点位（Overall Score） | 第一行：维度标识 第二行：百分位值 | Actuals - Internal Peers P52 |							
| 鼠标悬停指标分布条 | 指标名称、百分位值、同行规模 、具体的数值显示| ARR Growth Rate：P45 Peer Count: 45 Cohort Size |							
| 鼠标悬停 Trend 折线点 | 月份和所有交集点信息 | 2026-03 Actuals-Internal: P50 Actuals-KeyBanc: P55 |							
| 鼠标悬停雷达图点位 | 指标名称、维度、百分位 | Rule of 40 Actuals-Internal Peers: P62 |							
							
### 8.3 颜色编码规范							
							
**百分位等级与颜色对应**：							
							
红色   ← 0 ≤ n < 25%    Bottom Quartile							
粉色   ← 25 ≤ n < 50%   Lower Middle Quartile							
黄色   ← 50 ≤ n < 75%   Upper Middle Quartile							
绿色   ← 75 ≤ n ≤ 100%  Top Quartile							
灰色   ← N/A 或无数据							
							
							
**选中状态颜色**：							
						
蓝色点亮 ← VIEW、FILTER 筛选项被选中							
黄色点亮 ← DATA、BENCHMARK 筛选项被选中							
