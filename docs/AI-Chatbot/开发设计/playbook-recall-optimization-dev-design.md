...
>
> 阶段：⑤ 开发设计 ｜ 版本：**v3** ｜ 日期：2026-08-11 ｜ 状态：**已实现（sprint116）· 待审关系 diff 后发布** ｜ 端：Python（CIOaas-python）
>
> v3 相对 v2（实施后修订）：六项优化已在 sprint116 实现并通过独立审核（版本 2 回归 +0.0pt、
> 472 测试全绿、rerank 契约实调确认——原"上线前实调"项已闭环）。**两处认知修正**：① version 5
> 上现有金标测出的 r@1 99.68% 是**泄漏放大不是改进**（§6 注记），D-1 的真实效果尚无测量；
> ② 无泄漏金标从"D-2 的前置"升格为"量化 D-1 的必需品"（§2）。
>
...
⚠️ **2026-08-11 升格：D-1 落地后这件事从"D-2 的前置"变成"量化 D-1 的必需品"。** D-1 把这
315 条问法逐字变成了独立 question chunk，泄漏从"混在 profile 里"变成"每题都有原文副本"——
新版本上现有金标测出 r@1 99.68%，**纯属泄漏放大**（详见 §6 的实施后注记）。现有金标从此只剩
一个用途：**旧版本回归不许变**；D-1 / D-2 / C 术语路对真实用户问法的效果，都只有这套新金标
能量出来。

改法：用 `playbooks_bodies.json` 的**正文**让 haiku 出题（正文不在 profile 与 question chunk
的被检索文本内），脚本加 `--golden <file>` 支持外部金标文件。两套并用——旧套只做旧版本回归，
新套测真实水平与跨版本差值。
...
待人工动作 ───────────────> 审 version 5 的关系 diff → 决定是否 activate
                           （路由数字好看≠可发布，关系推断逐次不同，diff 是唯一闸门）
```
