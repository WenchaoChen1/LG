...
>
> 阶段：⑤ 开发设计 ｜ 版本：**v7** ｜ 日期：2026-08-25 ｜ 状态：**待评审** ｜ 端：Python（CIOaas-python）
>
> **v7 相对 v6（同日）：两条互相咬合的改动一起落地，生产口径从 r@3 变成 r@4。**
>
> ⑦ **`default_top_k` 3 → 4**：换到路由覆盖 **r@3 86.2% → r@4 89.3%（+3.1pt，多捞回 13 题）**，
> 难尾 82.6% → 86.6%。理由是两侧性质不同——那 13 题原先"对的手册压根不在 seed 里"、阶段 3
> 覆盖为 **0**（阶跃收益），代价只是每本正文变薄（渐变）。r@5 仅 89.8%（+0.5pt）故不再往上。
>
> ⑧ **`_MAX_EXCERPT_CHARS` 16000 → 22400**（= 缺省 top_k 4 × 配额 4 × 分块上限 1400，**一条算式
> 而非拍的数**）：**这是 ⑥⑦ 的必需补齐、不是可选项**。预算按 seed 均分，所以只做 ⑥⑦ 时每 seed
> 只剩 4000 字符、每本实交 **2.57 段——比两条改动之前的 2.68 段还低**，于是 v6 那条配额 3→4 变成
> 纯付成本、拿不到它测出来的收益（金标口径：2.69 段 ≈ recall 82.2%，3.36 段 ≈ 94.7%）。22400 下
> 每 seed 5600、实交 **3.32 段**（配额 4 的理论上限），⑥⑦ 才都兑现。
>
> **合起来**：总摘录量 9549 → 15473 字符（+62%），换到 r@4 89.3% + 每本 2.68 → 3.32 段。这是一笔
> 明确的**「拿 context 换覆盖」**交易，与优化项 A / F 压 context 的方向对冲，属产品取舍。累计量
> 风险已被跨调用去重压过一轮（`45e98bc4`，台账那次 52769 字符实测在它之前）。
>
> §1 基线表按 r@4 重排；§8 新增三条改动的合并说明（**单看任何一条都会读错**）；新增 §11 议题 9。
> ⚠️ 运行时读种子常量、不读 space 列，所以**已存在的那条 space 记录仍是 3、管理页会回显 3，
> 需人工改一次**。
>
> **v6 相对 v5（2026-08-25，多 reviewer 审查后修订）：v5 的两个结论都是在错误前提下下的。**
...
> ⚠️ **`default_top_k` 已于 2026-08-25 改为 4，生产口径随之变成 r@4**（见 §11 议题 9）；
> 本行是当时的记录，不再是现状。
>
...
| 指标 | 实测值 | 出处 / 口径 |
|---|---|---|
| **生产相关指标 r@3（旧口径）** | **96.5%**（304/315）→ **已饱和 100%** | ⚠️ **口径已变**：生产 `default_top_k` 2026-08-25 由 3 改为 4，现在的生产口径是 **r@4**（见下一行）。此行保留只为留档。**⚠️ 2026-08-24 复测：version=5 上此口径已到 r@1 99.7% / r@3 100%**（`d1-v5.json` 快照即已如此）——D-1 之后每条 questions 各自是一行 `chunk_kind='question'` 向量，拿它检索等于精确匹配自己。**该口径此后既测不出改进也测不出中等退化，已不可用**，改用下面的无泄漏口径 |
| **无泄漏口径 r@4（现行生产口径）** | **86.6%**（259/299） | 2026-08-25 重算，`baseline-leakfree-v5.json` 的 `rankByQuestion`（同一批 299 题，未重跑检索——只是把统计口径从 ≤3 改成 ≤4）。同批 r@3 82.6%、**r@5 仅 87.3%**（再加一个 seed 只 +0.7pt，收益枯竭） |
| **无泄漏口径 r@3（改动前的生产口径）** | **82.6%**（247/299） | 同上。**只覆盖"正文细节题"、单路检索、无 rerank**，读法见 §2。同批：r@1 64.2% / MRR@5 0.738 / 完全召不回 38 条 |
| **全量分层口径 r@4 / r@3** | **89.3% / 86.2%**（420 题） | `baseline-strata-v5.json`，难尾 299 + 主路 121。top_k 3→4 **多捞回 13 题（+3.1pt）**；这 13 题原先"对的手册压根不在 seed 里"、阶段 3 覆盖为 0，是上调 `default_top_k` 的直接理由（§11 议题 9） |
| **usable@3（新增）** | **75.6%**（226/299） | 同上。= 排进前三**且** top1 ≥ `COVERAGE_THRESHOLD`(0.50)。33 题（11%）检索找到了但线上会短路成"库里没这主题"——**只看 r@3 会系统性高估用户可见质量**。⚠️ 口径改 r@4 后这一项**尚未重算**（短路判据只看 top1 相似度、与 top_k 无关，所以 33 题这个数不变，但分母口径应同步改成 usable@4） |
...
⚠️ **收益会被总量闸门按 top_k 分档吃掉**（这也是同日把闸门从 16000 抬到 22400 的原因，见下）。
`_MAX_EXCERPT_CHARS` 按 seed 均分，而金标
...
| 3（改动前的缺省） | 5333 | 2.68 | 3.25 | **+0.57** |
| 4（现行缺省，**旧预算 16000**） | 4000 | 2.54 | 2.57 | +0.03 |

⚠️ **这三条改动必须一起读，单看任何一条都会读错**（2026-08-25 同日落地）：

| # | 改动 | 单独看它换到什么 | 单独看它的代价 |
|---|---|---|---|
| ⑥ | `EXCERPTS_PER_SEED` 3 → **4** | 每本手册的正文更厚 | 每 seed 多约 1.1k 字符 |
| ⑦ | `default_top_k` 3 → **4** | 路由 r@3 86.2% → **r@4 89.3%** | **把每本的预算摊薄了** |
| ⑧ | `_MAX_EXCERPT_CHARS` 16000 → **22400** | 让 ⑥ 真正兑现 | 单次摘录量 **+29%** |

**⑦ 会吃掉 ⑥**：预算按 seed 均分，多要一本就摊薄每本。只做 ⑥⑦ 不做 ⑧ 的话——

```
改动前  top_k=3 配额=3 预算16000 → 每 seed 5333 → 每本实交 2.68 段
只做⑥⑦ top_k=4 配额=4 预算16000 → 每 seed 4000 → 每本实交 2.57 段  ← 反而更薄
加上⑧  top_k=4 配额=4 预算22400 → 每 seed 5600 → 每本实交 3.32 段  ← 配额 4 的理论上限
```

而金标测出的对应关系是「2.69 段 → recall 82.2%」「3.36 段 → recall 94.7%」，所以**只做 ⑥⑦ 等于
让配额那条改动纯付成本、拿不到它测出来的收益**（2.57 段大致还在 82% 那一档）。⑧ 是必需的补齐，
不是可选的锦上添花。

**⑧ 的 22400 是一条算式，不是拍的数**：

```
缺省 top_k(4) × 每 seed 配额(4) × 单段上限(default_chunk_size = 1400) = 22400
```

三个因子都是硬上限——段再多也只取 4 段、段再长也不超切分器的 1400——所以它是**缺省口径下永远
够用的天花板，且与 playbook 内容多寡无关**（手册变长 / 变多都顶不到它）。⚠️ 再往上一个字都换不到
额外正文，只会抬高 `top_k=10` 那条暴走路径的上限，而那正是这个总量闸门要防的东西。
**改 ⑥⑦ 或 `default_chunk_size` 任一都要按上式重算**（配额或 top_k 任一加到 5 → 28000）。

**⑧ 的代价与它为什么现在可接受**：单次摘录量 12040 → 15473 字符（+29%）。台账「多轮检索累计灌给
模型 5 万字符」那条记的是 2026-07-31 实测（一轮连调 4 次、累计 52769 字符），⚠️ **但跨调用去重
是之后专门为它加的**（`45e98bc4`，`search_playbooks_tool._mark_already_provided`：本轮已给过正文的
pid 只留一行、清空 excerpts）。所以累计量现在主要由"不同手册的数量"决定而非调用次数，抬单次预算的
放大效应比台账那个时点小得多。

**合起来的净效果**：总摘录量 9549 → 15473 字符（+62%），换到 r@3 86.2% → r@4 89.3%（多捞回 13 题、
它们原先阶段 3 覆盖为 0）+ 每本正文 2.68 → 3.32 段。**这是一笔明确的"拿 context 换覆盖"的交易**，
与优化项 A / F 压 context 的方向对冲，属产品取舍。
...
```python
budget = _MAX_EXCERPT_CHARS      # 当时是 16000，在 for hit 循环【外面】初始化
for hit in hits:
...
| ~~9~~ | ~~playbook 的 `default_top_k` 取几~~ | ✅ **已定：4**（2026-08-25，由 3 上调，`process_type.py` PLAYBOOK 种子）。⚠️ **这是「宽 vs 深」的取舍，不是纯增益**——`_MAX_EXCERPT_CHARS`（16000）按 seed **均分**，多要一本手册就摊薄每本的正文：<br>• 用 ACTIVE 版 63 本真实段长实测（配额 `EXCERPTS_PER_SEED=4`）：**top_k=3 每 seed 预算 5333、实交 3.25 段 → top_k=4 预算 4000、实交 2.57 段**，每本薄掉 21%；<br>• 换来的路由覆盖：全量 420 题 **r@3 86.2% → r@4 89.3%（+3.1pt，多捞回 13 题）**，难尾 299 题 82.6% → 86.6%（+4.0pt）。<br>• **上调的理由是两侧的性质不同**：那 13 题原先"对的手册压根不在 seed 里"、阶段 3 覆盖为 **0**（阶跃收益）；而深度那一侧只是每本从 3.25 段变 2.57 段（渐变代价）。<br>• **不再往上**：r@5 仅 89.8%（+0.5pt，收益枯竭），而每本还要再薄一截。<br>• ⚠️ **想两头都要只能动 `_MAX_EXCERPT_CHARS`**，那与优化项 A/F 压 context 的方向对冲，属产品取舍。<br>• ⚠️ **运行时读的是种子常量、不是 space 列**：`space_service.create_space` 建空间时把种子值抄进 `ai_rag_space.default_top_k`，而 `playbook_service.search()` 走模块常量 `DEFAULT_TOP_K`。所以改种子**立即生效**，但已存在的那条 space 记录仍是 3、管理页会回显 3——**需要人工改一次那条记录**，否则照页面排查会被误导。 |
