...
>
> **v7 相对 v6（同日）：`default_top_k` 由 3 改为 4，生产口径随之从 r@3 变成 r@4。**
> 这是「宽 vs 深」的取舍——`_MAX_EXCERPT_CHARS` 按 seed 均分，多要一本手册就把每本的正文从
> 实交 3.25 段摊薄到 2.57 段；换来路由覆盖 r@3 86.2% → **r@4 89.3%（+3.1pt，多捞回 13 题）**。
> 上调的理由是两侧性质不同：那 13 题原先"对的手册压根不在 seed 里"、覆盖为 **0**（阶跃），
> 而深度那侧只是渐变变薄。r@5 仅 89.8%（+0.5pt）故不再往上。§1 基线表按新口径重排，新增
> §11 议题 9。⚠️ 运行时读种子常量、不读 space 列，所以**已存在的那条 space 记录仍是 3、管理页
> 会回显 3，需人工改一次**。
>
...
| **4（现行缺省）** | 4000 | 2.54 | **2.57** | **+0.03** |
| 5 | 3200 | 1.97 | 1.97 | **0** |
| 10 | 1600 | 1.05 | 1.05 | **0** |

⚠️ **`default_top_k` 已于同日改为 4（§11 议题 9），所以配额这一条在缺省路径上的净增只剩
+0.03 段**——两条改动互相干扰：配额想让每本更厚，而 top_k 摊薄了每本的预算。合起来看，
真正变化的是**分配方式**而非总量（总量恒被 `_MAX_EXCERPT_CHARS` 卡在 16000）：从"3 本 × 3.25 段"
变成"4 本 × 2.57 段"，用每本薄 21% 换 r@3 86.2% → r@4 89.3%。

**top_k ≥5 时配额的净增为 0** ——高 top_k 下真正的瓶颈从来不是配额，是这条总量闸门
（top_k=10 时每本只交付 1 段正文）。**想让配额那条重新生效，唯一的办法是抬 `_MAX_EXCERPT_CHARS`。**
