...
>
> 阶段：⑤ 开发设计 ｜ 版本：**v4** ｜ 日期：2026-08-25 ｜ 状态：**待评审** ｜ 端：Python（CIOaas-python）
>
> v4 相对 v3（2026-08-25，结构化 wiki 页做完并**否决**后修订）：新增 §2「⛔ 已实现但不接入：
> 结构化 wiki 页」——三代做法（提示词禁编造 → 逐行核出处 → 句级逐字拼装）与**三档对照**
> （纯 body 75% / wiki+body 74% / 页面单独 25%，配对 p=1.000）。结论是**不接入**：wiki 页没有
> 增加信息、只是重排信息，却占掉一个摘录槽位。同节留档三处口径教训（`lost 0` ≠ 覆盖安全 /
> 判卷对象必须是生产真正喂的东西 / **没有对照组的改进都是幻觉**）——这三条比 wiki 本身更有
> 复用价值。代码保留未接入，库里 wiki 分段 0 条、线上行为未变。
>
...
**动因**：阶段 3 喂给模型的是源站正文按 1400 字符切出来的散文段（每 seed 3 段约 3.0k 字符），
而段落级金标实测**每题平均只有 1.53 段真正回答问题**——大半预算花在行业感慨与引言铺垫上。
设想是给每本手册生成一页答案形状的结构化摘要（Goal / Key actions / Prerequisites / Outputs /
Metrics）取而代之，用约三分之一的字符承载同样的信息。

**结论先行：实测拿不出可测收益，不接入。** 代码与评测保留在树上（见文末位置），日后若做出
答案级评测、能证明"结构本身有用"，可以直接复用。

#### 三档对照（同一批 53 题、同一个判卷、`scripts/eval_playbook_wiki_coverage.py`）

| 喂给模型的摘录 | 覆盖 full | partial | lost |
|---|---|---|---|
| **纯 body 三段（今天线上）** | **75%**（40/53） | 25% | 0 |
| wiki 页 + 相似度最近的 2 段 body（接入后会是这样） | 74%（39/53） | 26% | 0 |
| wiki 页单独（**不是生产口径**，见下方教训②） | 25% | 75% | 0 |

配对检验（同一批题）：接入后 **5 题变好 / 6 题变差，McNemar p=1.000 —— 纯噪声**。

wiki 页没有增加信息，只是重排了信息；而它占掉一个摘录槽位，反而挤掉一段 body。金标标注的
相关段本来就大概率落在相似度前几名里，所以"body 太噪"这个前提在**可测的那个轴上**不成立。
...
| v3.x | **模型只回句号**，页面由原句逐字拼装 | **100%（构造保证，36/36 机械自检）** |
...
覆盖 full 从 60% 掉到 25%）——而这个代价在"页面 + body 段"的配置下被 body 段补掉，于是
才有了上面那张表里 74% 与 75% 的平手。

#### 三处口径教训（本项最有复用价值的产出）

① **`lost 0` 不等于"没丢信息"**。`partial` 的定义就是"丢了具体数字/步骤/角色"，而那恰是答案里
最有用的部分。实测 53 题里 27 题 partial，`audited-financials` 丢了 $2M 阈值、
`board-of-directors` 丢了 48 小时提前量与数据点清单、`license-register` 丢了季度频率——**每一条
都正是那道题问的东西**。看覆盖率只看 lost 会得出"安全"的错结论。
...
④（连带）**校验器与缓存自身也要被验证**：v2 的出处正则锚在行尾、只剥最后一个标记，模型一行
标两个出处时前一个残留、它的数字被当成"编造"整行丢掉——30 条丢弃**全是误杀、真编造 0 条**，
...
是否比三段散文更容易组织答案。那需要答案级评测（人工盲评或让模型比两版答案），成本比覆盖率
高一个量级。**所以严格说：wiki 页在"信息保全"上被证否，在"答案组织"上未被测量。**

#### 真正有收益的替代

段落级金标那一轮已经给出了更便宜的答案：**把摘录配额从 3 提到 4 是显著的**（12 题变好 /
0 题变差，p<0.001，recall 86.7%→95.0%、全覆盖 65.0%→76.7%），代价是每 seed 多约 800 字符
context。想改善阶段 3，这一行常量比造一个页面更值得先做——但它与优化项 A/F 压 context 的方向
对冲，属产品取舍（见 §11 议题 7）。
...
| `scripts/eval_playbook_wiki_coverage.py` | 覆盖 + 逐字自检，`--scope page+body\|page\|body` 三档 |
| `scripts/data/playbook/wiki-pages-v5.json` | 36 本的句号 JSON + 逐本句子指纹 |

**当前状态：库里 wiki 分段 0 条**（只在 ingest 时产出，且产的是 DRAFT）。线上行为至今未变；
`fetch_body` 对没有 wiki 分段的版本行为逐字不变，所以这些代码留在树上不影响生产。

**若要重启**：先做答案级评测（否则仍然只能量到"信息在不在"），再补齐 63 本的生成与人工审
（现只生成并审过 36 本），并处理 §11 遗留的几项（管理页耗时/成本文案已失真、缺按版本预览端点）。
