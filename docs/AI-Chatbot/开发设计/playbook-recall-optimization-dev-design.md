...
**顺带发现，已当场做掉 A/B**：在这套金标上**去掉 HyPE 的 question 分段、只留 profile 名片，
排序反而更准**——但 `usable@3` 掉 12.6pt。**结论是别删**，详见下方待做清单第 2 条的对照表。
旧金标看不到这件事，因为它已被 HyPE 打到饱和。

### ⏳ 仍待做（本项的剩余缺口，按性价比排序）

1. **多标签标注**：把金标结构从 `pid` 改成 `acceptable_pids[]`，优先处理代理指标里 74 条
   "对手本 body 相似度差距 ≤0.05" 的题。做完 r@1 才有意义（预计升 8~10pp，且是真实的）。
2. ~~**补"整本级 / 真实流量"分层**~~ **✅ 2026-08-24 完成（整本级部分）**：
   `golden-wholeplay-v5.json`（121 题，每本 2 条整本级问句，`stratum="whole-play"`）。
   本层**刻意不是无泄漏的**——线上真实检索词就是这个形态（实测 `how do I build an accurate
   cash flow forecast?` top1 **0.987**，而它本身就是索引问法之一），把词面重合消掉只会造出
   没人会问的题。评测脚本 `--golden` 现在**可重复传**以合并分层，一次运行出分层报告。
   ⚠️ **真实流量那半仍待人工定标**：`ai_rag_search_log` 里 25 条去重后的真实 CHATBOT 检索词
   已导出待确认（含当前检索前三与相似度），**标签不能由模型自己定**（等于自己给自己出题）。

   **分层基线（version=5，top_k=5，共 420 题，`baseline-strata-v5.json`）**

   | 分层 | n | r@1 | r@3 | usable@3 | top1 均值 |
   |---|---|---|---|---|---|
   | `body-detail`（难尾，正文细节题） | 299 | 64.2% | 82.6% | 75.6% | 0.607 |
   | `whole-play`（**线上主路**，整本级问法） | 121 | 78.5% | **95.0%** | **95.0%** | 0.679 |

   两个读数值得记住：① 主路 r@3 95% 而难尾 82.6%——**混成一个总分会让主路的退化整个看不见**；
   ② 主路的 `usable@3` 与 r@3 **完全相等**，即覆盖阈值的短路只咬难尾、不咬主路。

   **HyPE A/B（同口径两跑，只改 `_ROUTE_CHUNK_KINDS` 一个常量）**

   | | r@1 | r@3 | **usable@3** |
   |---|---|---|---|
   | A 现状（profile + question） | 68.3% | 86.2% | **81.2%** |
   | B 只留 profile 名片 | 75.5% | 87.6% | **68.6%** |
   | **B − A** | **+7.2pt** | +1.4pt | **−12.6pt** |

   **结论：不要拿 r@1 当理由删 HyPE 问法分段。** 去掉它排序确实更准（McNemar b=78 c=35
   p<0.001，净 +43 题），但**绝对相似度整体下沉**（难尾 top1 均值 0.607→0.539），大量正确命中
   掉到 `COVERAGE_THRESHOLD=0.50` 以下、线上被短路成"库里没这主题"。**HyPE 的真实贡献是把
   相似度顶到阈值以上，不是排序**；只看 r@1/r@3 会得出反向结论。
   ⚠️ 耦合提醒：0.50 这个阈值本来就是**带着 HyPE** 校准的（2026-08-12），所以 B 的 usable 崩塌
   有一部分是"没重校阈值"的产物——真要删 HyPE，**必须先按新分布重校阈值再评**。
...
### ✅ 已完成（2026-08-24）：阶段 3（取正文摘录）的段落级评测

**为什么单列一节**：以上全部内容都在评"阶段 1 找对手册没有"。而摘录是模型写答案的**原料**，
它以前**完全没有度量**——`tests/` 里的 `fetch_excerpts` 全是 mock，断言的是"摘录有没有被传下去 /
预算怎么切"这类接线。任何改摘录侧的动作（换选段策略、把正文换成 LLM 写的结构化页）都是盲改。

| 文件 | 内容 |
|---|---|
| `scripts/gen_playbook_excerpt_golden.py` | 段落级标注（每题标"哪几段正文真回答了它"，三档 answers/partial/no） |
| `scripts/data/playbook/excerpt-golden-v5.json` | 60 题 / 250 段判定（⚠️ **全部 `needsReview=true`**，模型初判） |
| `scripts/eval_playbook_excerpts.py` | precision / recall / 全覆盖 + **三档选段配置对照** + 配对显著性 |
| `scripts/data/playbook/baseline-excerpts-v5.json` | 基线快照 |

**口径三条**：① 只标**路由已命中（rank≤3）**的题——路由没命中时阶段 3 根本不会取这本正文；
② 候选 = 该题金标手册的**全部 body 分段**，不跨手册（跨手册是路由的问题）；③ **A 配置调的是
生产函数** `fetch_excerpts`，B/C 才用底层 `fetch_body` 自己选段，且每次运行有**口径自检**——
本地那份规则副本与生产实际输出不一致就直接报错退出，防"报出一个不存在的配置"。

结果与结论见 §11 议题 7（已被这套金标量化）。**当前限制**：60 题全部是模型初判、
`needsReview=true`，数字只能当方向性参考；另有 7 题"一段都没判相关"，需人工复核是判卷太严
还是金标标签本身可疑。
...
| ~~7~~ | ~~阶段 3 的"第一段固定带"是否保留~~ | ✅ **已量化（2026-08-24）：保留** —— 本文早先写"量不到"的结论已被推翻，段落级金标 + `eval_playbook_excerpts.py` 给出了数（60 题、`excerpt-golden-v5.json`，⚠️ **模型初判待人工确认**）：<br>• **body 第一段本身就是相关段的比例只有 25.0%** —— 原理由（"它是 Players/Effort/Frequency 元数据块"）确实已经死透；<br>• 但**去掉它并不显著更好**：B（纯相似度）配对下只在 **4/60** 题上有差异、`p=0.125`，**按噪声处理**。聚合均值看着 B 更高（precision 47.5% vs 45.3%、recall 90.5% vs 86.7%），**只看均值会误判** —— 所以结论仍是保留（零风险，不动）。<br>• **真正显著的是把配额 3→4**：C 配置 12 题变好 / 0 题变差（`p<0.001`），recall 86.7%→95.0%、全覆盖 65.0%→76.7%，代价是每 seed 多约 0.7 段（≈800 字符 context）。⚠️ 这与优化项 A/F 压 context 的方向对冲，**是个需要产品判断的取舍，不是纯技术优化**。<br>• 每题平均只有 **1.53** 段真正相关（正文平均 4.4 段），这解释了为什么 3 个名额多数够用、只在相关段 ≥3 时才捉襟见肘。<br><br>原记录（2026-08-11，已被上面取代）：降级为观察项。<br>背景：`fetch_excerpts` 固定带 body 第一段，原理由是"它是 Players / Effort / Frequency 元数据块，等于用 0 成本换掉两个数据库列"。但 2026-08-10 查库，这几个关键词在 63 个 entry 的第一段里**全部 0/63**（`Initial Effort` 在全部 body 分段里也是 0/63），现在第一段是开篇散文——**理由失效，但效果未必变差**：开篇段是"这本手册解决什么问题"的主题铺垫，本身有价值。<br>⚠️ **纠正本文早先的一处不准确**：曾写"可用现有金标量化"，其实**量不到**——评测脚本只跑 `route()`（阶段 1），摸不到 `fetch_excerpts`（阶段 3）；且现有 315 题的答案是"哪本手册"，不是"哪一段"。真要量化需另出一套段落级金标 + 人工对比三种配置（保留 / 不固定 / 配额加到 4 段），属答案质量评测，成本远高于收益。<br>另两个选项各自的代价：**改成不固定** → 丢掉主题铺垫，模型可能只看到中段细节；**配额加到 4 段** → 每 seed 3.4k → 4.5k 字符、3 seed 约 13.7k，与优化项 A / F 压 context 的方向对冲。真要做，等 A / F 落地、context 降下来之后再评估。<br>这件事**不影响路由**（元数据块从不进 profile 文本），也不影响 `category` / `stage`（`_meta_field` 单独解析、都健在）；连带暴露的校验缺口已转台账「ingest 缺"字段级静默变空"校验」 |
