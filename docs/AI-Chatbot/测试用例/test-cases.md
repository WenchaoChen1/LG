...
>
> 阶段：⑦ 测试用例 ｜ 日期：2026-08-11（v5 发布后更新 2026-08-12）｜ 端：Python（CIOaas-python）+ 前端聊天页
>
> 范围：**只覆盖本轮五项改动**（低置信短路 / 阶段并发 / 双检索词 / 每问一向量 / 精排），
> 不重复 playbook 既有功能（crawl / ingest / activate / 审核页）的用例。
> 文档里每一个**带数字的预期**（相似度、命中的手册、条数、耗时）都是 2026-08-11 / 08-12 **在本地库
...
2. **优化项 A 的阈值 0.45 与 D-1 是耦合的**：0.45 是按**改造前**的分数分布定的（那次"确实没覆盖"
   ...
   D-1 把合法口语化命中抬到 0.49~0.65 之后，0.45 才勉强站得住（仍不干净，见 §0.3）。
   > ⚠️ 这一条是本轮测试发现的、**设计文档两节都没预料到的耦合**：若日后回滚到 v2，
   > **A 的阈值必须一起往下调**，否则口语化问题会被整片吃掉。
...
### 1.1 ⚠️ 发布后立刻要盯的两类线上可见错误

这两类不是"将来可能"，是 §0.3 已实测、**现在线上就会发生**的：

| 现象 | 用户看到什么 | 实测样例 |
|---|---|---|
| **合法问题被误判成"库里没有"** | 明明有对应手册，却答"方法论库没有这个主题" | `we need to plan next year spending` 0.444、`we want a one page view of our key numbers` 0.395 |
| **库外问题被沾边手册硬答** | 拿 `employee-agreement` 回答 401k、拿 `contract-register` 回答租约 | 0.484 / 0.465 / 0.465 三条 |

碰到就按 §6 第 1 条**记原文 + top1 分数**，别当缺陷单提、也别改阈值——决策要等样本够。
