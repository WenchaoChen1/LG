...
| **D-1 的真实收益（量化）** | 金标就是那 315 条问法原文、现在**逐字命中**（相似度 1.000），r@1 99.7% 是泄漏 | 定性证据见 §0.2 与 §2.1 的口语化对照（v2 7/8 短路 → v5 5/8 命中第一）；**量化**需先建"由正文出题"的无泄漏金标（D-2 的前置）——**✅ 2026-08-24 已建**（`scripts/data/playbook/golden-leakfree-v5.json`，299 题；同口径 r@3 82.6% / usable@3 75.6%）。⚠️ 在该口径下 D-1 反而是**负收益**（去掉 question 分段 r@1 +7.0pp / r@3 +1.3pp），待单独 A/B，见召回优化设计 v3 §2 |
