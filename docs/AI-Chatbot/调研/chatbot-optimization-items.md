...
> 关联文档: [能力盘点与缺口分析](./capability-gap-analysis.md) · [调研任务书](./agent-optimization-research-brief.md) · [设计文档](../设计/design-doc.md)

> 产出日期: 2026-09-17
> 本文件按**收益由高到低**排列优化项。两份原始调研（现状实测 / 外部设计引进）已于 2026-09-20 并入本文并删除，基线数据见下方「基线实测」。
...
> 本地环境，样本 `chatbot.chat` 94 轮。**下面这些数字是全文所有收益估算的依据**，引用时请回到此处核对口径。
...
| retrieve 每轮 LLM 调用数 | 均值 **2.70**（1→7 轮, 2→38, 3→32, 4→9, 5→4, 6→1, 7→1） |
| 知识库检索轮次覆盖率 | **修复后 20/30 = 66.7%**（8 月 17/24 = 70.8%） |
| 工具调用失败率 | 25/576 ≈ **4.3%** |
| trace 终态 | SUCCESS 88 / PARTIAL 4 / RUNNING 2 |
| 固定前缀（系统提示 + 工具 schema） | ≈ **28.8k token**，其中**工具 schema 占 60%** |

> ⚠️ **已剔除的一类指标**：答案中断率、重新生成率、提问语言分布、指代类提问占比——这四项测的是**用户行为**，而开发库里的"用户"就是开发者本人。它们在开发样本上没有解读价值，不作为基线。上表各项测的是**系统与模型行为**，与谁在提问无关，因此可用。
...
| 曾经的说法 | 实际 | 为什么重要 |
|---|---|---|
| 系统提示词「337 行 / ~39KB」 | 那是**源文件**（含大量中文注释）。渲染给模型的系统提示 **17,520 字符**，工具 schema 序列化 **26,663 字符** | **长上下文的大头是工具 schema（60%），不是系统提示**——优化落点完全不同 |
| 缓存命中率 **0.8%** | **统计口径假象**。caching 于 **2026-09-04** 才接进流式 ReAct 路径，此前 245 次调用 cached 恒为 0 是结构性的；接线后有效样本仅 9 次 | **当前无可用命中率基线，需重新采集** |
| 知识库覆盖率 **42.4%** | 全样本聚合把改之前的数据混了进来——**67% 的样本（62/92）来自 2026-07-28 两次提示词修复之前**。按周拆：修复前 19/62 = **30.6%**，修复后 20/30 = **66.7%** | 论点从"最强措辞只换来 42%"降为"**修了两次才把 30% 拉到 67%，且仍未到 100%**"——仍成立，但强度要降 |

### 方法论总纲

> **凡是必须 100% 发生的动作，靠提示词只能逼近、不能保证。**
>
> 渲染后的提示词已有近 100 条硬命令（绝不×56、红线×12、⚠×13、必须×8、不得×7、严禁×2）。知识库那条已是全文最强措辞，修了两次才把覆盖率从 30.6% 拉到 66.7%，仍未到 100%。
>
> 所以措辞**不是无效**的——它的问题是**换不来确定性**，且加得越多、已有红线的相对强度越被稀释。这类诉求应优先找**结构化落点**（程序化注入 / 预置结果 / 尾部重述），提示词加规则是够用但不够格的手段。
>
> 这条是 §2、§6、§14.1 共同的出发点。
...
| 确定性单测 | 公司匹配函数、截断逻辑、去重不变式 | 纯逻辑，可 100% 覆盖 | 只覆盖程序化部分 |
| 线上代理指标 | §7-3 的 5 条 SQL + 新增 `ungrounded_number_count` | "有事发生"的报警 | 只说有事、不说是什么；**n=94 太小，现在不能接自动告警** |
...
| # | 事项 | 阻塞什么 |
|---|---|---|
| 1 | 工具返回体量上限与提示词现有编排规则的交互（"按上限拆成多次调用取全" vs "严禁撑大时间窗致服务端超时"），两条口径需对齐 | §9 二期 |
| 2 | **生产的非英文提问占比未知**（开发库的语言分布无参考价值，已剔除） | §12 常量表要预置哪几种语言；若生产几乎全英文则整条不做 |
| 3 | `ContextEditingMiddleware` 是并进 §9（源头截断），还是保留为 §5 收益落袋后的二期项 | §13 |

> 已拍板的不再列出：kb/combo 维持现状不清理、单公司语料按"会超过 20 万 token"规划、评测集做折中版（15~20 条、只标槽位）、playbook 常开。

### 诚实标注的证据空白

- **「首 token 前该允许几次 LLM 往返」没有任何公开基准或业界惯例数字。** §1 引用的外部材料只提供"guardrail 可并行"这一**机制性**证据。但**落点已用我们自己的数据核验**（各节点实测耗时 + TTFT 下界 + `input_guard` 在 **0/76** 轮次里成为瓶颈），所以 §1 的结论有本地实证，只是"业界该有几跳"这个普适问题无外部答案。
- **澄清式反问的收益/代价没有公开量化数据。** open_deep_research 与 OpenAI Cookbook 都只给"怎么做"，没给"反问后答案质量提升多少 / 用户流失多少"。§3 必须灰度自测，不能拿业界数字背书。
- **数字型 citation 的效果数据不存在**——只有产品形态描述（cell-level citation、lineage、审计要求），没有可信的准确性/信任度实测。§14 的定价因此只能靠 §14.1 的探针先取基线。
- **财务/受监管领域的 agent 记忆**没有专门的公开工程实践，最接近的是纵向安全预印本。
- **「会话内上下文可见性」与「对话式答案定点纠错」的效果**均未找到可靠外部证据（§3 的两个支柱）。

### 检索失败 / 未能证实（如实记录）

- **OpenRouter 对 `cache_control.ttl="1h"` 的官方逐字说明**未找到（`/endpoints` 返回 `input_cache_write_1h` 价格，但文档正文无参数说明）→ **§5 上线前需实发一次请求验证**
- **OpenRouter 对 `anthropic-beta: context-management-2025-06-27` 的官方支持/否定声明**均未找到，只有社区证据
- **"含 rewriter 时 TTFT 增加 2.4×"**（网传出自 RAGO）—— 原文无此句，**不予采信**
- **arXiv 2601.16720《Watching AI Think》** —— 核实后发现它测的是 empathy/warmth/competence/engagement，**不测等待感知**，不予引用
- **CodeCompass「工具采纳率约 38%」** —— 属代码领域，仅作旁证。⚠️ 初稿曾说"我们的 42.4% 独立复现了同一量级"，**该说法已作废**（见基线实测的三个已作废数字）
- **中文系统提示 + 英文输出、独立改写跳 vs 合并首轮** —— 未找到针对这两个具体组合的实测研究
...
31. [NVIDIA NeMo Guardrails — Streaming / Output rails](https://docs.nvidia.com)（v0.23）— input rails 先于任何 token 产出；`stream_first` 的取舍
32. [Masking Stale Observations… Until It Doesn't](https://arxiv.org/abs/2606.00408)（2026-05-29）— 遮蔽收益呈非对称倒 U 形，**强模型饱和区 sharp collapse**（我们属风险侧）
33. [Speculative Actions](https://arxiv.org/abs/2510.04371) — 预测准确率最高 55% → 延迟最高 −20%。⚠️ 环境非财务取数
34. [Beyond English: Prompt Translation Strategies](https://arxiv.org/html/2502.09331)（35 语言 × 4 任务）— **instruction 部件语言偏好最弱**。⚠️ 测试模型早于当前代际

**弱证据 / 仅作形态参考（已标注，未用于支撑量化结论）**
35. NN/g《Less Chat, More Answer》（n=9 定性）、《5 Dimensions of Site-Specific AI Chatbots》
36. 金融分析产品（Daloopa 等）的 cell-level citation 形态 — 产品观察
