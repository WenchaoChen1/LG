...
| 5 | kb / combo 两条不可达智能体轨 | **纳入"砍掉以降低维护面"的候选** | 需评估砍除后的收益（提示词/测试/分支维护面）与风险（API 直调方是否存在） |
...
- ~~`_AnswerEchoFilter` token 双发~~ → **根因已修，见 §3.1**。

---

## 3. 立即可做（无需调研，已定位）

### 3.1 下线 `_AnswerEchoFilter`（根因已修，补丁是遗留物）— ✅ 已于 2026-09-16 执行完毕

> 本节保留为决策记录。代码、单测与相关文档已清理，**待真实环境流式回归验证**；调研执行者不必再排期这项。

**结论**：token 双发的两个来源都已根治，该过滤器现在是空转的遗留补丁。

| 来源 | 现状 |
|---|---|
| 外层 `DBRouterChatModel._astream` 手动上报 token | 已不再手动调 `on_llm_new_token`（方法 docstring 明确声明） |
| 内层 langchain 模型 run 经 contextvars 继承 ambient 回调再报一遍 | ① `openai_compat`（含 OpenRouter）自 `e3750aea` 起传输层迁 **openai 原生 SDK**，无 Runnable / 无回调概念，问题**结构性消失**；② `anthropic_native` 仍走 ChatAnthropic，但 `69875ede`（2026-07-02）起显式传 `ISOLATED_CALLBACKS_CONFIG` 隔离，从源头只发一份 |

遗留原因：计划 `docs/superpowers/plans/2026-07-02-trace-llm-remaining-work-plan.md` 条目 3-1 包含"修根因 + 删过滤器"两件事，根因当天就修了（同日提交），**删过滤器这半件没做，计划文档也未更新状态**。

**保留它的实际风险**（不只是死代码）：过滤器按 metadata 对象身份锁定"首个产文 run"，只在遇到 tool_call chunk 时 `reset`。一旦出现"同一段答案中途换了 run 对象但中间没有工具调用"的合法场景，它会**静默丢弃**后续全部 token，且无任何日志或指标。

**清理范围**：`retrieval_agent.py` 的类定义与调用点、`retrieval_kb_agent.py` 的 import 与调用点、`tests/ai/chatbotgraph/test_answer_echo_filter.py`、`source/ai/CLAUDE.md` 与 `docs/AI-Chatbot/实现与业务流转.md` 的相关描述、计划文档 3-1 的状态。

**验证**：真实环境回归一次流式对话（无重复、tool_step 正常），OpenRouter 与 anthropic 两条 provider 路径各测一次。

### 3.2 评估砍除 kb / combo 两条轨
...
| # | 待定事项 | 阻塞什么 |
|---|---|---|
| 1 | kb / combo 是否存在 API 直调方 | §3.2 砍除决策 |

> §3.1（下线 `_AnswerEchoFilter`）已于 2026-09-16 执行完毕，待真实环境流式回归验证。
