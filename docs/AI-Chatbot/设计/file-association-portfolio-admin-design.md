...
设计据此把触发时机定为 **文件入队之后、点发送之前**。该窗口早于登记、早于向量化、早于摘要生成，**比 PRD 字面要求更严格**地满足 AC1「在任何记忆处理开始之前」，且是唯一可行的窗口。

> **触发点取"入队"而非"上传就绪"**（2026-08-11 调整）：推断只看会话历史 + 草稿、**与文件内容无关**，没有理由等文件传完——大文件传几十秒时归属条会一直空着。入队即发让推断与上传并行，归属条更早出现，用户也就有更长时间核对/改正，这正是 D3「推断即生效」最需要的缓解。比 presign 成功还早一步，且前端拿不到 presign 的精确回调（`onChip` 只暴露 status/percent）。
> 两处代价，均已接受：① 上传失败时这次推断白花（概率低、无害）；② 此刻用户往往还没打字，故 `draft` 常为空——已有会话里历史消息本就是主要信号，草稿只是补充。
...
| D2 | 推断能力建在哪 | **新增推断接口**，前端在文件入队时调用（与上传并行，见 §一） |
| D3 | AI 推断填入后用户未操作即发送 | **算已选中，直接生效** |
| D4 | 发送后是否回显归属 | **不回显**（2026-08-11 定）：输入框内不留任何提示，见 §6.5 |
...
3. ~~发送后保留一条 inline 提示~~ —— D4 已定为不回显，故**发送后不再有任何补救提示**，缓解全部落在发送前的归属条上（见 §6.5 的取舍说明）。
...
8. **超时预算 = 10s**，超时即按空结果返回。
   > 初版按直觉定的 3s，**实测被打脸**：`ai_llm_call_log` 里 195 次 haiku 调用 min 1.0s / avg 2.05s / max 7.4s，3s 只覆盖 85%——约七分之一的正常调用会被自己砍掉（本地实测第一次就撞上 `llm_timeout`）。10s 覆盖全部历史样本再留余量。
   > 放宽不牺牲体验：用户从上传到点发送通常远超 10s；超时只是归属条留空、由用户手选，不阻断任何流程；万一响应回来时用户已发送，前端批次 token 会丢弃它（§6.4）。
   > 超时调用**会**落 `ai_llm_call_log`（`status=FAILED`、`perf_elapsed_ms≈超时值），超时率可从看板查。但 `ai_trace_span` 里该 span 的 `status` 仍是 `SUCCESS`——服务降级返回空结果、并未抛异常，判断是否超时要看 span 属性 `infer_outcome`。
...
```
用户选择文件 → chip 入队（推断与上传并行发起）→ 直传 S3 → chip 变 ready
    │
...
发送后 chips 被清空、归属条随之消失，**输入框内不保留任何归属提示**。

曾实现过一版"发送后在输入框上方留一条 inline 提示"，实测后废弃（2026-08-11）。两个问题：

1. **它脱离了消息上下文**。提示挂在输入框上方而非跟着那条消息，于是与之后每一轮对话共存——用户接着问几个不带附件的问题，那句"1 file added to X's memory."仍挂在那里，说的却是几轮之前的事。
2. **要修好它就得改契约**。让提示跟着消息走，需要把 `file_company_id` 随消息存储（当前不存，`Chat.tsx` 已有注释确认），那是 §7.3 明确划在范围外的消息侧契约改动。

**代价（明确接受）**：D3「推断即生效」的补救窗口收窄到**发送前**——归属条在用户点发送前一直可见，那是唯一的核对时机。发送后要核对，去 Knowledge Base / Memory Settings 面板（那里有 Portfolio/Company 标签与删除入口），或按 §三 D3 的纠错路径删除重传。
...
| AC | 满足方式 | 备注 |
|----|----------|------|
| 1 | §4.1 推断接口在文件入队后、记忆写入前调用 | 触发时机见偏差 1 |
...
| 9 | §7.4 已实现 | |
| 10 | §6.2 发送前提示（发送后无回显，见 §6.5） | 未归属的告知只在发送前成立 |
