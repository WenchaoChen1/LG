...
- 操作流程说明：
  ...
  - 如果题库有更新，且有未完成草稿，进入草稿时，会有弹窗提示，题库已更新
     按钮：提交草稿内容，回答新题库
     提交草稿就提交现有的记录后展示新题库（因为有新题库，这个草稿提交只是进历史记录，无法实际使用）
     回答新题库就直接刷掉草稿展示新题库
  - 问卷提交人已最终提交者为准
- 全端响应式。
...
- 权限：GSV 团队均可见，可以点击Share按钮分享给Founder端。若分享后又重新提交内容，有新的建议生成，founder端的内容就置空，直到下次分享
...
- **Dimension Configuration tab**
  ...
  - Dimension configuration中，若某个Dimension已经关联了历史版本题目，则此demension不可被删除，仅可进入deactive板块，以防历史数据有问题。
---
