...
| calculateType | 条件（`financial_growth_rate` 有效记录数） | 预测方式               | Projection Basis     |
| ------------- | ---------------------------------- | ------------------ | -------------------- |
| 1             | < 6 条                             | Formula（纯同业）       | `PEER_BASED_FORMULA` |
| 2             | 6–23 条                            | Formula（自身 + 同业混合） | `HYBRID_FORMULA`     |
| 3             | ≥ 24 条                            | AI Model（Ensemble） | `ENSEMBLE_MODEL`     |

> 此处"有效记录数"指 `financial_growth_rate` 表中该公司最近 24 个月（截至 close month）的记录条数，不等于 `finance_manual_data` 的原始月份数（revenue 为 null 的月份不会生成 growth_rate 记录）。`calculateType` 是代码内部变量（1/2/3），与 `financial_forecast_current.type` 数据库字段（"0"/"1"/"2"）不同。
...
1. **Manual 模式**：按公司、版本号、日期范围从表 `finance_manual_data` 取数，同一日期有多条时取 `updated_at` 最新的一条（SQL 使用 `ROW_NUMBER() OVER (PARTITION BY date ORDER BY updated_at DESC)` 实现），再做货币换算后返回。
2. **Automatic 模式（cover=true，纯自动）**：通过 ETL 接口按公司与日期查 Redshift 表（`QuickbooksProfitAndLoss` / `QuickbooksBalanceSheet`，由 cio-bigdata 定时从 QuickBooks 写入），换算后返回，不写入表 `finance_manual_data`。
3. **Automatic 模式（cover=false，混合查询）**：保留切换前的 Manual 历史数据。按 `company_quickbooks.update_mode_at` 为分界线：
   - `update_mode_at` **之前**的月份 → 查 `finance_manual_data`（Manual 数据）
   - `update_mode_at` **之后**的月份 → 查 Redshift（Automatic 数据）

> `cover` 字段来自 `company_quickbooks` 表：`true` = 完全覆盖（纯 Automatic），`false` = 保留 Manual 历史数据（混合查询）。
