...
开发 → 审核 → 测试 → 提交，每个环节之间**必须停下找用户确认**，不得自动串联：

- **先执行开发**：收到开发指令后直接把开发做完（含编写单测代码与轻量校验 py_compile/tsc/lint）。
- **开发完不自动审核**：开发完成后在回复末尾提醒"有改动待审核"，**等用户确认后**才启动代码审核（含多 agent 审查、规范自查）。审核时机=git 提交之前统一做一次。
- **审核产出的修复**：按审核结论修复属审核环节的一部分，随审核一并执行。
- **测试同理**：跑测试前找用户确认（详见「测试运行策略」）。
- 本条**覆盖**各子项目 CLAUDE.md 中「每次对话完成开发工作后，必须用规范审查一遍代码」的自动自查要求——该审查合并到提交前审核环节，由用户确认后执行。

## 优化项台账（强制）

三个主工程各自在 `docs/` 下维护两份台账，由 Claude 自动维护、无须用户提醒：

- `docs/待优化项.md`：排查/开发过程中**识别出但本次不处理**的优化点（技术债、依赖升级、性能隐患、存量坏文件等），识别出来即追加一条：`- **标题**（识别日期）：一两句问题与建议方案（来源）`。
- `docs/已完成优化.md`：某个待优化项完成后，从待优化项中删除该条，并在此追加：`- **标题**（完成日期）：改动摘要（涉及文件/提交）`。

适用范围：`CIOaas-api/docs/`、`CIOaas-web/docs/`、`CIOaas-python/docs/`。写入前先查重，同一事项更新原条目而不是新增。
