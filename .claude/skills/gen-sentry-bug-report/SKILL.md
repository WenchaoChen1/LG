...
1. **Sentry MCP 已注册并连接**：执行 `claude mcp list` 应能看到 `sentry: ✓ Connected`。
...
8. **commit message 用中文**（符合项目 CLAUDE.md 规范）。
...
claude mcp add sentry -s user -- cmd /c npx -y '@sentry/mcp-server' --access-token=sntryu_<TOKEN>
```
